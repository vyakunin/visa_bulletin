...
def benchmark_full_processing(
    filepath: Path, visa_program: str, sample_size: int = None
) -> dict:
    ...
    if fiscal_year is None:
        ...
        print(
            f"Warning: Could not extract fiscal year from filename, using current year: {fiscal_year}"
        )
    ...
    print(
        f"  File reading: {read_time:.2f} seconds ({read_time / total_time * 100:.1f}%)"
    )
    print(
        f"  Row processing: {parse_time:.2f} seconds ({parse_time / total_time * 100:.1f}%)"
    )
...
def main():
    ...
    parser.add_argument("--file", "-f", type=Path, help="Path to file to benchmark")
    parser.add_argument(
        "--program",
        "-p",
        choices=["h1b", "perm"],
    ...
    parser.add_argument(
        "--sample-size", type=int, help="Limit to N rows for faster benchmarking"
    )
    parser.add_argument(
        "--all-large",
        action="store_true",
    ...
    if args.all_large:
        ...
        for filepath in data_files[:5]:
            visa_program = (
                VisaProgram.PERM if "perm" in filepath.name.lower() else VisaProgram.H1B
            )
            result = benchmark_full_processing(
                filepath, visa_program, sample_size=10000
            )
        ...
        print("=" * 80)
        print(
            f"{'File':<50} {'Rows':<12} {'Time (s)':<12} {'Rows/sec':<12} {'Mem (MB)':<10}"
        )
        print("-" * 100)
