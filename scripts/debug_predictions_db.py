...
def debug_predictions(year, month):
    ...
    try:
        bulletin = PredictedBulletin.objects.get(
            target_bulletin_month__year=year,
            target_bulletin_month__month=month
        )
        ...
        for cutoff in bulletin.cutoffs.all():
            if cutoff.visa_class in matrix:
                if cutoff.country in matrix[cutoff.visa_class]:
                    ...
                    if cutoff.country == 1:
                        print(f"  Mismatch Country: {cutoff.country} (type {type(cutoff.country)})")
                        print(f"  Available keys: {list(matrix[cutoff.visa_class].keys())}")
            else:
