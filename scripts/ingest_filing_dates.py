...
def ingest_filing_dates_from_file(filepath: Path, bulletin: Bulletin) -> int:
    ...
    logger.info(f"Processing {filepath.name} (bulletin from {bulletin.publication_date})")
    ...
    # Extract cutoff data
    extractor = TableToCutoffData(bulletin.publication_date, bulletin.url or str(filepath))
...
def main():
    ...
    final_action_count = VisaCutoffDate.objects.filter(action_type='final_action').count()
    logger.info(f"\nDatabase totals:")
