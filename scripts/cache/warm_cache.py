...
def warm_market_overview_cache(verbose: bool = False):
    ...
    if verbose:
        logger.info(
            f"    Total filings: {stats['basic'].get('total_filings', 'N/A'):,}"
        )
        logger.info(
            f"    Median salary: ${stats['basic'].get('median_salary', 0):,.0f}"
        )
...
def warm_count_caches(verbose: bool = False):
    ...
    base_qs = (
        SalaryRecord.objects.exclude(is_worksite=True)
...
def warm_directory_caches(verbose: bool = False):
    ...
    # Top employers count
    top_employers_count = (
        EmployerCluster.objects.exclude(canonical_name="Unknown")
    ...
    top_job_titles_count = JobTitleCluster.objects.exclude(
...
def main():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed cache contents"
    )
    parser.add_argument(
        "--market-only", action="store_true", help="Only warm market overview cache"
    )
    parser.add_argument(
        "--base-url",
        type=str,
