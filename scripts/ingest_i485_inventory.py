...
def normalize_country(c_str: str) -> int:
    ...
    if "INDIA" in c_upper: return Country.INDIA
    if "CHINA" in c_upper: return Country.CHINA
    if "PHILIPPINES" in c_upper: return Country.PHILIPPINES
    if "MEXICO" in c_upper: return Country.MEXICO
    if "SALVADOR" in c_upper or "GUATEMALA" in c_upper: return Country.EL_SALVADOR_GUATEMALA_HONDURAS
    return Country.ALL

def normalize_visa_class(v_str: str) -> str:
    v_upper = v_str.strip().upper()
    if "1ST" in v_upper or "EB-1" in v_upper: return "1st"
    if "2ND" in v_upper or "EB-2" in v_upper: return "2nd"
    if "3RD" in v_upper or "EB-3" in v_upper: return "3rd"
    if "OTHER" in v_upper or "EW" in v_upper: return "3rd" # treat EW as 3rd for now or separate? VQS uses "3rd" generic usually but expects specific if needed.
...
def main():
    ...
    parser.add_argument("--pub-date", help="Publication date (YYYY-MM-DD)", required=True)
