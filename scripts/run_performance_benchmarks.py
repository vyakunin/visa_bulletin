...
def benchmark_excel_methods(filepath: Path, sample_rows: int = 5000):
    ...
    print("-" * 80)
    print(f"{'iterrows() (OLD)':<30} {old_time:<15.3f} {old_rate:<20,.0f} {'1.0x (baseline)':<15}")
    print(f"{'to_dict() (NEW)':<30} {new_time:<15.3f} {new_rate:<20,.0f} {f'{speedup:.1f}x':<15}")
    print()
...
def generate_side_by_side_comparison(baseline, excel_bench, import_results=None):
    ...
    print("-" * 80)
    print(f"{'Metric':<40} {'Before (iterrows)':<25} {'After (to_dict)':<25} {'Improvement':<20}")
    print("-" * 110)
    ...
    print(f"{'Processing rate (rows/sec)':<40} {old_rate_val:,.0f}{'':<18} {new_rate_val:,.0f}{'':<18} {speedup_val:.1f}x faster{'':<8}")
    print(f"{'Time per 10k rows (seconds)':<40} {old_time_10k:.2f}{'':<20} {new_time_10k:.2f}{'':<20} {time_speedup:.1f}x{'':<15}")
    print()
    
    print("IMPORT PERFORMANCE (Baseline vs Optimized)")
    print("-" * 80)
    print(f"{'Metric':<40} {'Baseline (from logs)':<25} {'Optimized (if tested)':<25} {'Improvement':<20}")
    print("-" * 110)
    ...
    if import_results:
        ...
        print(f"{'Average import rate (rec/sec)':<40} {baseline_rate:,.0f}{'':<18} {new_rate:,.0f}{'':<18} {improvement_pct:+.1f}%{'':<13}")
        print(f"{'Total import time (for 618k rows)':<40} {baseline_time_618k:.0f}s{'':<20} {new_time_618k:.0f}s{'':<20} {time_improvement_pct:.1f}% faster{'':<8}")
    else:
        print(f"{'Average import rate (rec/sec)':<40} {baseline['import_rate_avg']:,.0f}{'':<18} {'(run --test-import)':<25} {'':<20}")
        print(f"{'Early batch rate (rec/sec)':<40} {baseline['import_rate_early']:,.0f}{'':<18} {'(run --test-import)':<25} {'':<20}")
        print(f"{'Late batch rate (rec/sec)':<40} {baseline['import_rate_late']:,.0f}{'':<18} {'(run --test-import)':<25} {'':<20}")
        print(f"{'Slowdown as DB grows':<40} {baseline['slowdown_pct']:.1f}%{'':<20} {'(check logs)':<25} {'':<20}")
    ...
    print(f"{'Excel read time (618k rows)':<40} {baseline_excel_time:.0f}s{'':<20} {expected_excel_time:.0f}s{'':<20} {excel_speedup:.1f}x faster{'':<8}")
    print(f"{'Overall import time (618k rows)':<40} {baseline_total:.0f}s{'':<20} {expected_total:.0f}s{'':<20} {overall_speedup:.1f}x faster{'':<8}")
    print()
    ...
    if import_results:
        print(f"✓ Actual import rate: {import_results['import_rate']:,.0f} records/second")
    print(f"✓ Database timing now visible (bulk vs commit)")
    print(f"✓ Can identify bottlenecks accurately")
    print()
...
def main():
    ...
    parser.add_argument(
        '--max-rows',
        type=int,
        help='Limit import to N rows for testing'
    )
    ...
    if args.file:
        ...
        if args.test_import:
            ...
            visa_program = VisaProgram.PERM if 'perm' in filename_lower else VisaProgram.H1B
    ...
    if excel_bench:
        ...
        print("Example: python3 scripts/run_performance_benchmarks.py --file dol_data/file.xlsx")
