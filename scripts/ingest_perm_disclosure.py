...
def normalize_visa_class(row: dict, extra_mappings: dict) -> str:
    ...
    if "BACHELOR" in edu_upper:
        ...
        try:
            exp_months = int(float(get_column_value(row, extra_mappings['experience_months']) or 0))
            if exp_months >= 60:
                return "2nd"
        except:
            pass
...
def process_file(filepath: Path, publication_date: date):
    ...
    EXTRA_MAPPINGS = {
        'country': ['COUNTRY_OF_CITIZENSHIP', 'Country_of_Citizenship', 'FW_INFO_CTRY_OF_CIT'],
        'education': ['FOREIGN_WORKER_INFO_EDUCATION', 'Foreign_Worker_Info_Education', 'MINIMUM_EDUCATION'],
        'experience_months': ['JOB_INFO_EXPERIENCE_NUM_MONTHS', 'Job_Info_Experience_Num_Months', 'REQUIRED_EXPERIENCE_MONTHS'],
    }
