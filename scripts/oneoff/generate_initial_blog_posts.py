...
def _get_model_metrics() -> dict[ComparedModel, ModelMetrics]:
    ...
    Last evaluated: 2026-07-27 (§26 re-measure, staging prod-copy DB).

    Dir% is the aggregate `direction_acc` at 6m (any non-zero actual move), NOT
    `cond_direction_acc` (>30d moves). Its null baseline on this data is ~92% —
    see §26; the column is kept for comparability with community claims, with the
    baseline stated in the prose beneath the table.

    §26 re-measure moved two rows: THIS_MODEL 164→177 composite / 198→209 6m
    (the §25 tuned-GBM graduation is the leading explanation — it was validated on
    the publish-path conditional objective, not this MAE composite) and
    DEMAND_SUPPLY_QUEUE 193→201 / 227→241 (I-140 ledger growth). Persistence, Pace,
    Dashboard, Seasonal and Momentum reproduced their prior values to within 0.4d
    on the same harness and window, which is what makes the two moves attributable
    to the models rather than the evaluation.
    """
    raw: dict[ComparedModel, ModelMetrics] = {
        ...
        ComparedModel.DEMAND_SUPPLY_QUEUE: ModelMetrics(
            ...
            cond_dir_pct=65.0,
            source_html=(
                "Queue model; worse point estimate. Its Dir% is the highest in this table but still"
                " below the ~92% an always-guess-advance rule scores, because the model is structurally"
                " incapable of predicting a retrogression."
        ...
        ComparedModel.THIS_MODEL: ModelMetrics(
            ...
            source_html=(
                ...
                " Beats Persistence at 6m (209d vs 229d avg)."
            ),
        ),
    }
...
def _build_methodology_content() -> str:
    ...
    section4_html = (
        ...
        "  This model wins on both composite (177d vs next-best Pace 180d) and 6m MAE (209d vs 213d),\n"
        ...
        "  is trivially correct are excluded). Read this column with its baseline in mind: across these six\n"
        "  series since 2016, <strong>92% of the months that moved were advances</strong> (232 of 253), so the\n"
        "  one-line rule \u201calways guess advance\u201d scores about 92% here \u2014 higher than every model in the table,\n"
        "  including mine. What that rule cannot do is call a retrogression, which is the move that actually\n"
        "  costs people time, and it carries no magnitude at all. Direction accuracy on its own is a weak\n"
        "  benchmark on this data; the composite and 6m MAE columns are the ones worth comparing.\n"
        "</p>"
    )
