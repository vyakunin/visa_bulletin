...
def run_refined_comparison():
    print(
        "--- Starting Refined Model Comparison (Iter 2): System vs Linear Extrapolation ---"
    )
    ...
    for pb in preds:
        ...
        for key in categories_to_test:
            ...
            history = (
                ...
                .filter(bulletin__publication_date__lte=knowledge_date)
            ...
            for h in reversed(valid_history):
                m_idx = (
                    h.bulletin.publication_date.year * 12
                    + h.bulletin.publication_date.month
                )
    ...
    for group in ["NOISY", "REST"]:
        ...
        print(
            f"{group:<10} {'MAE':<10} {s_mae:<10.1f} {l_mae:<10.1f} {s_mae - l_mae:<10.1f}"
        )
        print(
            f"{'':<10} {'MedAE':<10} {s_med:<10.1f} {l_med:<10.1f} {s_med - l_med:<10.1f}"
        )
        print(
            f"{'':<10} {'MSE':<10} {s_mse:<10.0f} {l_mse:<10.0f} {s_mse - l_mse:<10.0f}"
        )
        print("-" * 60)
