...
def debug_eb3_row():
    ...
    print(f"Latest Prediction Set: {latest_pb.target_bulletin_month} (Created: {latest_pb.generated_at})")
    ...
    if pred_cutoff:
        ...
        print(f"  Confidence Interval: {pred_cutoff.confidence_low} - {pred_cutoff.confidence_high}")
    ...
    for h in reversed(history):
        if not h.cutoff_date: continue
        
        # Approximate month index
        months_diff = (h.bulletin.publication_date.year - 2020) * 12 + h.bulletin.publication_date.month
