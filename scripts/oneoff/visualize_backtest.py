...
# Artifact Directory
ARTIFACT_DIR = "/Users/vyakunin/.gemini/antigravity/brain/fe663a4c-c9d2-4347-b11c-7b9414a10399"
...
def generate_dashboard_forecast_at_horizon(visa_class, country, action_type, target_date, months_prior):
    """
...
def generate_vqs_forecast_at_horizon(visa_class, country, action_type, target_date, months_prior):
    """
...
def generate_static_forecast_at_horizon(visa_class, country, action_type, target_date, months_prior):
    """
    ...
    latest = VisaCutoffDate.objects.filter(
        visa_class=visa_class,
        country=country,
        action_type=action_type,
        bulletin__publication_date__lte=knowledge_date
    ).order_by('-bulletin__publication_date').first()
...
def run_lagged_chart_multi_series():
    ...
    if RawFactsLedger.objects.exists():
        earliest_fact = RawFactsLedger.objects.order_by("publication_date").first().publication_date
        print(f"VQS Data available from: {earliest_fact}")
    else:
        print("WARNING: No RawFactsLedger data found. VQS will yield flat lines (persistence).")
    ...
    for country, visa_class, label in SERIES_TO_ANALYZE:
        ...
        for d in plot_dates:
            ...
            # Dashboard
            p = generate_dashboard_forecast_at_horizon(visa_class, country, action_type, d, 6)
            ...
            # Static (Persistence) 
            p = generate_static_forecast_at_horizon(visa_class, country, action_type, d, 6)
    ...
    webapp_template_path = os.path.join(settings.BASE_DIR, 'webapp', 'templates', 'spaghetti.html')
