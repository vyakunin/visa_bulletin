...
def debug_vqs(visa_class, country, action_type, target_date, months_prior):
    ...
    target_facts = RawFactsLedger.objects.filter(
        dimensions__country=country, dimensions__category=visa_class
    ).count()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--visa",
        choices=["India", "China", "Mexico", "Philippines", "All"],
        default="India",
    )
    parser.add_argument(
        "--category", choices=["1st", "2nd", "3rd", "4th", "5th"], default="3rd"
    )
    parser.add_argument(
        "--action", choices=["final_action", "filing"], default="filing"
    )
