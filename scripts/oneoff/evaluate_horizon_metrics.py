...
def evaluate_group(name, keys, horizons, action_type, start_date, end_date):
    ...
    while sim_date < end_date:
        ...
        for visa_class, country_name in keys:
            ...
            for h_months in horizons:
                ...
                # Get Actual
                actual = get_actual_cutoff(visa_class, country, action_type, target_date)
                if not actual:
                    continue
                ...
                recent_points = [
                    (d, c) for d, c in zip(history_dates, history_cutoffs) 
                ...
                if len(recent_points) >= 2:
                    ...
                    months_diff = (last[0].year - first[0].year)*12 + (last[0].month - first[0].month)
                    if months_diff < 1: months_diff = 1
                    rate = (last[1] - first[1]).days / months_diff
                    
                    if rate > 0:
                        months_to_target = (target_date.year - sim_date.year)*12 + (target_date.month - sim_date.month)
                        add_days = rate * months_to_target
                        dashboard_pred = last[1] + datetime.timedelta(days=int(add_days))
                    else:
                         dashboard_pred = predict_linear(visa_class, country, action_type, sim_date, target_date)
                else:
                     dashboard_pred = last_known_cutoff

                # --- C. Linear Model (24-mo Regression) ---
                linear_pred = predict_linear(visa_class, country, action_type, sim_date, target_date)
                
                if dashboard_pred:
                    errors[h_months]["Dashboard"].append(abs((dashboard_pred - actual).days))
                if linear_pred:
    ...
    print(f"{'Horizon':<10} {'Dashboard':<12} {'Linear (24m)':<15} {'Naive':<10} {'Best'}")
    print("-" * 60)
    ...
    for h in horizons:
        ...
        best_model = ""
        if best_score == mae_d: best_model = "Dashboard"
        elif best_score == mae_l: best_model = "Linear"
        else: best_model = "Naive"
        
        print(f"{h} Month{'s' if h>1 else ' ':<4} {mae_d:<12.1f} {mae_l:<15.1f} {mae_n:<10.1f} {best_model}")
...
def run_horizon_evaluation():
    ...
    evaluate_group("DYNAMIC Series (India/China EB2/3)", dynamic_keys, horizons, action_type, start_date, end_date)
    evaluate_group("STATIC Series (ROW EB1/4)", static_keys, horizons, action_type, start_date, end_date)
