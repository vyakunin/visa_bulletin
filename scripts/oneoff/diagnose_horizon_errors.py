...
def main():
    ...
    print(
        f"Evaluating Baseline at Horizon {horizon} for {len(all_bulletins)} bulletins..."
    )

    rows = compute_bulletin_accuracy(
        bulletins=all_bulletins, meta=base_params, horizon=horizon
    )
    ...
    print(
        f"{'Series':<20} | {'Bulletin':<12} | {'Pred':<12} | {'Actual':<12} | {'Error'}"
    )
    print("-" * 75)
    for r in valid_rows[:20]:
        series = f"{r.visa_class}/{r.country}"
        print(
            f"{series:<20} | {str(r.bulletin_date):<12} | {str(r.predicted_cutoff):<12} | {str(r.actual_cutoff):<12} | {r.error_days} days"
        )
