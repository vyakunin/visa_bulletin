...
def benchmark_methods(filepath: Path, sample_rows: int = 5000):
    ...
    print()
    print(
        f"{'Metric':<40} {'iterrows() (OLD)':<25} {'to_dict() (NEW)':<25} {'Improvement':<20}"
    )
    print("-" * 110)
    print(
        f"{'Processing rate (rows/sec)':<40} {old_rate:>23,.0f} {new_rate:>23,.0f} {speedup:>18.1f}x faster"
    )
    print(
        f"{'Time per 10k rows (seconds)':<40} {10000 / old_rate:>23.2f} {10000 / new_rate:>23.2f} {speedup:>18.1f}x faster"
    )
    print(
        f"{'Time for 618k rows (seconds)':<40} {618000 / old_rate:>23.0f} {618000 / new_rate:>23.0f} {speedup:>18.1f}x faster"
    )
    print()
    ...
    if len(records_old) == len(records_new):
        matches = sum(
            1
            for i in range(min(10, len(records_old)))
...
def main():
    ...
    parser.add_argument(
        "--file", "-f", type=Path, required=True, help="Path to Excel file to benchmark"
    )
    parser.add_argument(
        "--sample-rows",
        type=int,
