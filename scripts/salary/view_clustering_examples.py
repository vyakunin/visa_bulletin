...
def format_example(example: dict, index: int, show_details: bool = True) -> str:
    ...
    lines.append(f"\n{'='*80}")
    lines.append(f"Example #{index} {type_icon} [{example_type}] {truth_icon} [{ground_truth}]")
    lines.append(f"{'='*80}")
...
def format_markdown_table(examples: list[dict]) -> str:
    ...
    lines.append("\n# Clustering Examples\n")
    lines.append("| # | Type | Truth | Similarity | Employer 1 | Employer 2 | Location 1 | Location 2 |")
    lines.append("|---|------|-------|------------|-------------|------------|-----------|-------------|")
    ...
    for i, ex in enumerate(examples, 1):
        ...
        lines.append(f"| {i} | {ex_type} | {truth} | {sim_str} | {emp1} | {emp2} | {loc1} | {loc2} |")
...
def main():
    ...
    parser.add_argument(
        ...
        help='Filter by ground truth'
    )
    parser.add_argument(
        '--limit',
        type=int,
        help='Limit number of examples to display'
    )
    parser.add_argument(
        '--min-similarity',
        type=float,
    ...
    parser.add_argument(
        '--max-similarity',
        type=float,
        help='Maximum similarity score to include'
    )
    parser.add_argument(
        '--output',
        type=Path,
    ...
    script_logger.log_call(
        args=vars(args),
        context='Viewing clustering examples in human-readable format'
    )
    ...
    if args.filter_truth:
        examples = [ex for ex in examples if ex.get('ground_truth') == args.filter_truth]
        logger.info(f"After truth filter: {len(examples)} examples")

    if args.min_similarity is not None:
        examples = [ex for ex in examples
                   if ex.get('similarity') is not None and ex.get('similarity') >= args.min_similarity]
        logger.info(f"After min similarity filter: {len(examples)} examples")

    if args.max_similarity is not None:
        examples = [ex for ex in examples
                   if ex.get('similarity') is not None and ex.get('similarity') <= args.max_similarity]
