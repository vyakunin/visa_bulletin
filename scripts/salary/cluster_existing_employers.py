...
CHECKPOINT_FILE = os.environ.get(
    "CLUSTERING_CHECKPOINT_FILE", "/tmp/clustering_checkpoint.json"
)
    ...
    try:
        ...
        with transaction.atomic():
            ...
            ClusteringCheckpoint.objects.bulk_create(
                checkpoint_entries, ignore_conflicts=True
            )

        logger.debug(
            f"Checkpoint updated: +{len(phase1_new)} phase1, +{len(phase2_new)} phase2"
        )
...
def load_checkpoint() -> dict | None:
    ...
    try:
        ...
        phase1_processed = set(
            ClusteringCheckpoint.objects.filter(phase="phase1").values_list(
                "item_key", flat=True
            )
        )
        phase2_processed = set(
            ClusteringCheckpoint.objects.filter(phase="phase2").values_list(
                "item_key", flat=True
            )
        )
...
def clear_checkpoint(checkpoint_file: str = CHECKPOINT_FILE):
    ...
    try:
        ...
        logger.info(
            f"Checkpoint cleared: {deleted_count:,} entries removed from database"
        )
    except Exception as e:
...
class ProgressTracker:
    ...
    def log_progress(self, batch_size: int = 1000):
        ...
        if self.pbar:
            ...
            should_log = pairs_since_last_log >= 1000 or time_since_last_log >= 5.0
...
def _load_and_group_employers(
    shuffle: bool = False, seed: int | None = None, limit: int | None = None
    ...
    qs = (
        ...
        .only("id", "name", "city", "state", "canonical_cluster")
    )
    ...
    if shuffle:
        ...
        if limit:
            ...
            logger.info(
                f"Selected {len(all_employers):,} random employers (limit={limit})"
            )
        else:
    ...
    logger.info(
        "Grouping employers by normalized name (re-normalizing from original names)..."
    )
...
def _process_same_normalized_name_matches(
    ...
    if checkpoint:
        ...
        if phase1_processed:
            logger.info(
                f"Resuming Phase 1: {len(phase1_processed):,} already-processed normalized names"
            )
    ...
    unprocessed_employers = {
        k: v
        for k, v in employers_by_normalized.items()
    ...
    if checkpoint:
        ...
        logger.info(
            f"Phase 1: Will process {phase1_total:,} pairs ({phase1_count:,} already done)"
        )
    else:
    ...
    try:
        for normalized_name, employers in employers_by_normalized.items():
            ...
            if (
                checkpoint_file
                and names_processed_since_checkpoint >= checkpoint_interval
            ):
                # Save batched new items incrementally (append-only, no full file rewrite)
                save_checkpoint_incremental(
                    pending_checkpoint_names, set(), checkpoint_file
                )
...
def _build_lsh_index(
    normalized_names: list[str], threshold: float = 0.7
    ...
    for idx, norm_name in enumerate(normalized_names):
        ...
        if (idx + 1) - last_log >= log_interval:
            ...
            logger.info(
                f"  Processed {idx + 1:,}/{len(normalized_names):,} names "
    ...
    elapsed = time.time() - start_time
    logger.info(
        f"LSH index built in {elapsed:.1f}s ({len(normalized_names):,} names, "
...
def _process_cross_normalized_name_matches(
    ...
    logger.info(
        f"Found {len(candidate_pairs):,} candidate normalized name pairs via LSH in {query_elapsed:.1f}s"
    )
    logger.info(
        f"  (Compared to {len(normalized_names) * (len(normalized_names) - 1) // 2:,} total pairs - "
    ...
    if checkpoint:
        ...
        if phase2_processed:
            logger.info(
                f"Resuming Phase 2: {len(phase2_processed):,} already-processed candidate pairs"
            )
    ...
    try:
        ...
        for norm1, norm2 in candidate_pairs:
            ...
            candidate_progress.log_progress(
                batch_size=1000
            )  # Log every 1k candidate pairs
            ...
            if (
                checkpoint_file
                and pairs_processed_since_checkpoint >= checkpoint_interval
            ):
                # Save batched new items incrementally (append-only, no full file rewrite)
                save_checkpoint_incremental(
                    set(), pending_checkpoint_pairs, checkpoint_file
                )
    ...
    logger.info("\nPhase 2 complete:")
    logger.info(
        f"  Candidate pairs processed: {candidate_summary['processed']:,}/{candidate_summary['total']:,}"
    )
    logger.info(f"  Employer pairs processed: {processed_pairs:,}")
    logger.info(f"  Time: {candidate_summary['elapsed_seconds']:.1f}s")
    logger.info(
        f"  Rate: {candidate_summary['rate_per_second']:.0f} candidate pairs/sec"
    )
...
def cluster_existing_employers(
    ...
    if checkpoint_file:
        ...
        if checkpoint:
            ...
            logger.info(
                f"  Resuming from checkpoint (Phase 1: {phase1_count:,} names, "
    ...
    global \
        _process_employer_pair_output_file, \
        _process_employer_pair_min_pairs, \
        _process_employer_pair_pairs_collected
    pairs_file = None
    ...
    if min_pairs_needed is not None:
        ...
        logger.info(
            f"Early stopping enabled: Will stop when {min_pairs_needed:,} pairs collected"
        )
    else:
    ...
    try:
        ...
        all_employers, employers_by_normalized = _load_and_group_employers(
            shuffle=shuffle_employers, seed=shuffle_seed, limit=employer_limit
        )
...
def main():
    ...
    parser.add_argument(
        "--min-pairs",
        type=int,
        help="Stop early when this many pairs collected (for fast testing)",
    )
    parser.add_argument(
        "--shuffle",
        action="store_true",
        help="Shuffle employers before processing (for random sampling)",
    )
    parser.add_argument(
        "--shuffle-seed",
        type=int,
    ...
    parser.add_argument(
        "--limit-employers",
        type=int,
        help="Limit number of employers to process (for fast debugging)",
    )
    parser.add_argument(
        "--stats-only",
        action="store_true",
    ...
    if args.reset_clustering:
        ...
        with transaction.atomic():
            ...
            updated_count = Employer.objects.exclude(
                canonical_cluster__isnull=True
            ).update(canonical_cluster=None)
    ...
    checkpoint_file = (
        ...
        if args.resume or args.checkpoint_file != CHECKPOINT_FILE
        else None
    )
    if args.resume and not checkpoint_file:
        checkpoint_file = CHECKPOINT_FILE

    script_logger.log_call(args=vars(args), context="Cluster existing employers")
