...
script_logger.log_call(
    args={},
    context='Reviewing pending bucket mismatch reviews'
)
...
for i, review in enumerate(reviews, 1):
    ...
    if norm1 == norm2:
        ...
        # Check for typos
        if any(typo in norm1 or typo in norm2 for typo in ['consltancy', 'servies', 'seperation', 'innocations', 'corportaion']):
        ...
        if words1 and words2 and words1[0] != words2[0]:
            logger.info(f"   → DIFFERENT: Different first words ({words1[0]} vs {words2[0]})")
            to_reject.append((review, f"Different first words: {words1[0]} vs {words2[0]}"))
        else:
