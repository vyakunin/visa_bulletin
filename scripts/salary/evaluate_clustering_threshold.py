...
def run_clustering(
    threshold: float, pairs_output: str, min_pairs: int, seed: int
) -> bool:
    ...
    cmd = [
        ...
        pairs_output,
        "--min-pairs",
        str(min_pairs),  # Stop when we have enough pairs for sampling
        "--shuffle",  # Shuffle for random sampling
        "--shuffle-seed",
        str(seed),  # Reproducible randomness
    ]
    ...
    if result.returncode != 0:
        ...
        if (
            "Enough pairs collected" in result.stderr
            or "Enough pairs collected" in result.stdout
        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Test clustering threshold and report precision/recall"
    )
    parser.add_argument(
        "--threshold", type=float, required=True, help="Threshold to test"
    )
    parser.add_argument(
        "--sample-size", type=int, default=500, help="Sample size per category"
    )
    parser.add_argument(
        "--min-pairs",
        type=int,
    ...
    parser.add_argument(
        "--output-dir", default="/tmp/clustering_test", help="Output directory"
    )
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument(
        "--show-all-samples",
        action="store_true",
        help="Show all false positive/negative samples",
    )
    ...
    if ollama_check.returncode != 0:
        ...
        logger.error(
            "Heuristic validation is circular (uses same thresholds we're testing)."
        )
    ...
    if "llama3.2:3b" not in model_check.stdout and "mistral" not in model_check.stdout:
        ...
        pull_result = subprocess.run(
            ["ollama", "pull", "llama3.2:3b"], capture_output=True, text=True
        )
    ...
    total_skipped = (
        metrics["auto_clustered"]["skipped"] + metrics["queued_for_review"]["skipped"]
    )
    if total_skipped > len(auto_sample) * 0.1:  # More than 10% skipped
        logger.warning(
            f"WARNING: {total_skipped} samples skipped ({total_skipped / len(auto_sample) * 100:.1f}%). "
    ...
    if false_positives:
        ...
        with open(md_file, "w") as f:
            ...
            f.write(
                f"Generated from evaluation with sample size: {len(auto_sample)} auto-clustered pairs\n\n"
            )
            f.write("---\n\n")
            ...
            for i, fp in enumerate(false_positives, 1):
                ...
                f.write(
                    f"- Location: {fp.get('emp1_city', 'N/A')}, {fp.get('emp1_state', 'N/A')}\n\n"
                )
                f.write(f"**Employer 2:** {fp['emp2_name']}\n")
                f.write(
                    f"- Location: {fp.get('emp2_city', 'N/A')}, {fp.get('emp2_state', 'N/A')}\n\n"
                )
