...
def main():
    ...
    for cluster in JobTitleCluster.objects.filter(slug__isnull=False).order_by(
        "-total_filings"
    )[:50]:
        ...
        record_count = SalaryRecord.objects.filter(
            job_title_entity__in=job_titles, wage_annual__isnull=False
        ).count()

        if record_count > 0:
            clusters_with_data.append(
                (cluster.slug, cluster.canonical_title, record_count)
            )
