  ...
  List all LCA files:
    ...
    if args.file:
        ...
        if is_perm:
            ...
            print(f"{'=' * 80}")
            print("  wage_from: ['WAGE_OFFER_FROM_9089', 'WAGE_OFFERED_FROM_9089', 'JOB_OPP_WAGE_FROM']")
            ...
            print(f"{'=' * 80}")
            print("  wage_from: ['WAGE_RATE_OF_PAY_FROM', 'LCA_CASE_WAGE_RATE_FROM', 'WAGE_RATE_OF_PAY_FROM_1']")
            print("  wage_to: ['WAGE_RATE_OF_PAY_TO', 'LCA_CASE_WAGE_RATE_TO', 'WAGE_RATE_OF_PAY_TO_1']")
        ...
        if not found_wage:
            ...
            if is_perm:
                print("  2. Update PERM_COLUMN_MAPPINGS in lib/parsing/salary/db_importer.py")
            elif is_lca:
                print("  2. Update LCA_COLUMN_MAPPINGS in lib/parsing/salary/db_importer.py")
                print("  3. Check if WAGE_RATE_OF_PAY (singular) needs special parsing (may contain ranges)")
        ...
        for filename in top_files[:args.top_missing]:
            ...
            if filepath.exists():
                ...
                if 'error' not in result:
                    print(f"File: {result['filename']}")
                    print(f"Wage-related columns: {len(result['wage_related_columns'])}")
                    if result['wage_related_columns']:
