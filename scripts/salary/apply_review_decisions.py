    ...
    for example_num, (
        ...
        try:
            ...
            if not emp1 or not emp2:
                logger.warning(
                    f"Example #{example_num}: Employer not found (emp1: {emp1 is not None}, emp2: {emp2 is not None})"
                )
                not_found += 1
                continue
            ...
            status = "APPROVED" if should_approve else "REJECTED"
            logger.info(
                f"Example #{example_num}: {status} - {emp1_name} vs {emp2_name}"
            )
