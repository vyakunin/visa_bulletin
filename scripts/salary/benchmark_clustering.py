...
def load_examples(jsonl_file: Path, only_reviewed: bool = True) -> list[dict]:
    ...
    if only_reviewed:
        logger.info(
            f"Filtered to {len(examples)} hand-reviewed examples (from {jsonl_file})"
        )
    else:
...
def run_production_benchmark(
    ...
    for ex in examples:
        ...
        pairs.append(pair)
        employers1.append(
            create_mock_employer(
                ex["emp1_name"], ex.get("emp1_city", ""), ex.get("emp1_state", "")
            )
        )
        employers2.append(
            create_mock_employer(
                ex["emp2_name"], ex.get("emp2_city", ""), ex.get("emp2_state", "")
            )
        )
    ...
    lsh, minhashes = _build_lsh_index(
        list(all_normalized_names), threshold=lsh_threshold
    )
    ...
    if skipped_different_buckets > 0:
        logger.warning(
            f"  ⚠️  {skipped_different_buckets} pairs not found by LSH (would not be compared in production Phase 2)"
        )
        # Categorize by ground truth
        bucket_mismatch_same = [
            m for m in all_bucket_mismatches if m["ground_truth"] == "same"
        ]
        bucket_mismatch_different = [
            m for m in all_bucket_mismatches if m["ground_truth"] == "different"
        ]
        logger.warning(
            f"      - {len(bucket_mismatch_same)} pairs marked as 'same' (false negatives - LSH didn't find them)"
        )
        logger.warning(
            f"      - {len(bucket_mismatch_different)} pairs marked as 'different' (true negatives - correctly not compared)"
        )

        # Show examples of false negatives (same company not found by LSH)
        if bucket_mismatch_same:
            logger.warning(
                "      Examples of false negatives (same company, LSH didn't find):"
            )
            for case in bucket_mismatch_same[:5]:
                reason = case.get("reason", "Not found by LSH")
                logger.warning(
                    f"        - '{case['emp1_name']}' ({case['normalized1']}) vs '{case['emp2_name']}' ({case['normalized2']}) - {reason}"
                )
        ...
        if bucket_mismatch_different:
            logger.info(
                "      Examples of true negatives (different companies, not found by LSH - expected):"
            )
            for case in bucket_mismatch_different[:5]:
                logger.info(
                    f"        - '{case['emp1_name']}' ({case['normalized1']}) vs '{case['emp2_name']}' ({case['normalized2']})"
                )
            if len(bucket_mismatch_different) > 5:
                logger.info(
                    f"        ... and {len(bucket_mismatch_different) - 5} more"
                )
    ...
    for i, (outcome, is_same_ground_truth, pair) in enumerate(
        zip(outcomes, ground_truth, pairs)
    ):
    ...
    logger.info(
        f"    Convert pairs: {timing['convert_pairs']:.3f}s ({timing['convert_pairs'] / len(pairs) * 1000:.1f}ms/pair)"
    )
    logger.info(f"    Ground truth: {timing['ground_truth']:.3f}s")
    logger.info(
        f"    Production algorithm: {timing['verification']:.3f}s ({timing['verification'] / len(pairs):.3f}s per pair)"
    )
    ...
    if bucket_mismatch_cases:
        logger.warning(
            f"  ⚠️  {len(bucket_mismatch_cases)} false negatives due to LSH not finding similar pairs:"
        )
        logger.warning(
            "      These pairs would never be compared in production Phase 2 (LSH didn't find them)"
        )
        logger.warning(
            "      This indicates LSH may need tuning or normalization needs refinement"
        )
    ...
    logger.info(
        f"  ⚠️  Bottleneck: {max_phase[0]} ({max_phase[1]:.3f}s, {max_phase[1] / execution_time * 100:.1f}% of total)"
    )
...
async def run_llm_benchmark(
    ...
    logger.info(f"    Ground truth: {timing['ground_truth']:.3f}s")
    logger.info(
        f"    LLM verification: {timing['verification']:.3f}s ({timing['verification'] / len(pairs):.3f}s per pair)"
    )
    logger.info(f"    Metrics calculation: {timing['metrics_calc']:.3f}s")
    logger.info(f"    Total: {execution_time:.3f}s")
    logger.info(f"    Throughput: {pairs_per_second:.1f} pairs/second")
    ...
    return BenchmarkResult(
        ...
        prompt_template=verifier.config.prompt_template
        or str(verifier.config.prompt_template_path)
        or "default",
        total_pairs=len(pairs),
        execution_time_seconds=execution_time,
        true_positives=true_positives,
...
def print_results(results: list[BenchmarkResult]):
    ...
    # Header
    print(
        f"{'Config':<20} {'Model':<15} {'Precision':<10} {'Recall':<10} {'F1':<10} "
    ...
    for r in results:
        print(
            f"{r.config_name:<20} {r.model:<15} {r.precision:<10.3f} {r.recall:<10.3f} "
    ...
    for r in results:
        ...
        print(
            f"  Prompt: {r.prompt_template[:50]}..."
            if len(r.prompt_template) > 50
            else f"  Prompt: {r.prompt_template}"
        )
        ...
        if r.false_positive_cases:
            ...
            for i, fp in enumerate(r.false_positive_cases[:5], 1):
                print(
                    f"    {i}. '{fp['emp1_name']}' vs '{fp['emp2_name']}' (similarity: {fp['similarity']:.3f})"
                )
        ...
        if r.false_negative_cases:
            ...
            for i, fn in enumerate(r.false_negative_cases[:5], 1):
                print(
                    f"    {i}. '{fn['emp1_name']}' vs '{fn['emp2_name']}' (similarity: {fn['similarity']:.3f})"
                )
...
async def start_ollama_server() -> subprocess.Popen | None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if available_models:
                logger.info(
                    f"Ollama server already running with {len(available_models)} models (checked in {elapsed:.3f}s)"
                )
                return None
            else:
                logger.warning(
                    f"Ollama server running but has no models (checked in {elapsed:.3f}s)"
                )
                logger.warning(
                    "  This may be a Bazel-provided server with empty models directory"
                )
    ...
    try:
        ...
        logger.info(
            f"Started Ollama server (PID: {process.pid}, spawn: {spawn_elapsed:.3f}s)"
        )
        ...
        for i in range(10):  # Wait up to 10 seconds (reduced from 30)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"✓ Ollama server is ready (wait: {wait_elapsed:.3f}s, total: {total_elapsed:.3f}s)"
                    )
                    return process
            except Exception:
                if i % 2 == 0:
        ...
        total_elapsed = time.perf_counter() - start_time
        logger.error(
            f"Ollama server failed to start within 10 seconds (wait: {wait_elapsed:.3f}s, total: {total_elapsed:.3f}s)"
        )
        ...
        total_elapsed = time.perf_counter() - start_time
        logger.error(
            f"Failed to start Ollama server (total: {total_elapsed:.3f}s): {e}",
            exc_info=True,
        )
        return None
...
async def ensure_model_available(model: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if model in models:
                total_elapsed = time.perf_counter() - start_time
                logger.info(
                    f"✓ Model {model} is already available (cached) (check: {check_elapsed:.3f}s, total: {total_elapsed:.3f}s)"
                )
                return True
    except Exception as e:
        check_elapsed = time.perf_counter() - check_start
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info(
                f"✓ Successfully pulled model {model} (pull: {pull_elapsed:.2f}s, total: {total_elapsed:.2f}s)"
            )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if model in models:
                        logger.info(
                            f"✓ Model {model} verified in Ollama after pull (verify: {verify_elapsed:.3f}s)"
                        )
                        return True
                    else:
                        logger.warning(
                            f"⚠ Model {model} not found in /api/tags after pull (verify: {verify_elapsed:.3f}s)"
                        )
                        logger.warning(f"  Available models: {models}")
                        logger.warning(
                            f"  This may indicate Ollama is not using OLLAMA_MODELS={models_dir}"
                        )
                        # Still return True - model was pulled, may work anyway
                        return True
                else:
                    logger.warning(
                        f"⚠ Failed to verify model after pull (status {response.status_code})"
                    )
                    return True  # Assume success if pull succeeded
            except Exception as e:
                verify_elapsed = time.perf_counter() - verify_start
                logger.warning(
                    f"⚠ Failed to verify model after pull (verify: {verify_elapsed:.3f}s): {e}"
                )
                return True  # Assume success if pull succeeded
            ...
            logger.warning(
                f"Failed to pull model {model} via Bazel (pull: {pull_elapsed:.2f}s, total: {total_elapsed:.2f}s)"
            )
            logger.warning(
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if model in available_models:
                        logger.info(
                            f"✓ Model {model} is available via Ollama server (even though Bazel pull failed)"
                        )
                        return True
                    else:
                        # Try pulling via system ollama CLI (not Bazel)
                        logger.info(
                            f"Model {model} not in server. Attempting to pull via system ollama CLI..."
                        )
                        import shutil
                        ...
                        if ollama_cli:
                            ...
                            if pull_result.returncode == 0:
                                ...
                                if response2.status_code == 200:
                                    ...
                                    available_models2 = [
                                        m.get("name")
                                        for m in models_data2.get("models", [])
                                    ]
                                    if model in available_models2:
                                        logger.info(
                                            f"✓ Model {model} successfully pulled via system ollama CLI"
                                        )
                                        return True
                        logger.error(
                            f"✗ Model {model} is NOT available in Ollama server"
                        )
                        logger.error(f"  Available models: {available_models}")
                        return False
            except Exception as e:
                logger.error(
                    f"Failed to check model availability after pull failure: {e}"
                )
                return False
        ...
        total_elapsed = time.perf_counter() - start_time
        logger.error(
            f"Timeout pulling model {model} (pull: {pull_elapsed:.2f}s, total: {total_elapsed:.2f}s)"
        )
        return False
    except Exception as e:
        pull_elapsed = time.perf_counter() - pull_start
        total_elapsed = time.perf_counter() - start_time
        logger.error(
            f"Error pulling model {model} (pull: {pull_elapsed:.2f}s, total: {total_elapsed:.2f}s): {e}",
            exc_info=True,
        )
        return False
...
async def main_async(args):
    ...
    if args.mode in ("llm", "both"):
        ...
        if server_elapsed > 15.0:
            logger.error(
                f"Server startup took {server_elapsed:.3f}s (exceeded 15s limit)"
            )
        ...
        for model in set(models_to_check):  # Remove duplicates
            ...
            if not model_available:
                logger.error(
                    f"Model {model} is not available and could not be pulled. Exiting."
                )
    ...
    try:
        ...
        if not examples:
            ...
            if args.only_reviewed:
                logger.error(
                    "  Try --include-all-types to include auto-clustered and different-company pairs"
                )
            return
        ...
        if args.mode == "production":
            ...
            if not args.skip_default:
                ...
                result = await run_llm_benchmark(
                    verifier, examples, "default", limit=args.limit
                )
                results.append(result)
            ...
            for name, template in prompt_templates.items():
                ...
                result = await run_llm_benchmark(
                    verifier, examples, name, limit=args.limit
                )
                results.append(result)
            ...
            if ollama_process is None:
                ...
                if not args.skip_default:
                    ...
                    result = await run_llm_benchmark(
                        verifier, examples, "llm-default", limit=args.limit
                    )
                    results.append(result)
...
def main():
    ...
    script_logger.log_call(args=vars(args), context=context)

    # Run async main with timeout
    try:
        result = asyncio.run(
            asyncio.wait_for(main_async(args), timeout=args.max_total_time)
        )
        if result is None:
