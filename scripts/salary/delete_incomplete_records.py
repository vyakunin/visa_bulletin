...
def delete_incomplete_records(dry_run: bool = True) -> dict:
    ...
    logger.info(f"Found {salary_drop_count:,} SalaryRecord and {worksite_drop_count:,} WorksiteRecord records to delete")

    if dry_run:
        logger.info(f"[DRY RUN] Would delete {salary_drop_count:,} SalaryRecord and {worksite_drop_count:,} WorksiteRecord records")
        return {
            'salary_deleted': 0,
            'worksite_deleted': 0,
            'dry_run': True
        ...
        with transaction.atomic():
            ...
            logger.info(f"Deleted {salary_deleted:,} SalaryRecord and {worksite_deleted:,} WorksiteRecord records")
            return {
...
Examples:
  ...
  Actually delete records:
    ...
    script_logger.log_call(
        args=vars(args),
        context='Deleting incomplete records (missing salary data)'
    )
