...
def harvest_bucket_mismatch_candidates(
    ...
    employers = list(
        Employer.objects.all().values("id", "name", "name_normalized", "city", "state")
    )
    ...
    logger.info(
        "  Using optimized approach: word-based bucket indexing + random sampling"
    )
    ...
    logger.info(
        f"  Built word index: {len(word_to_buckets)} core words mapping to buckets"
    )
    logger.info(
        f"  (Excluded generic words like: {', '.join(sorted(list(ALL_GENERIC_WORDS))[:10])}...)"
    )
    ...
    logger.info(
        f"  Sampled {total_sampled} employers from {len(buckets)} buckets (1-2 per bucket)"
    )
        ...
        for bucket2_name in candidate_buckets:
            ...
            for emp1 in emp1_list:
                ...
                for emp2 in emp2_list:
                    ...
                    if (
                        not core_words1.intersection(core_words2)
                        and len(core_words1) > 0
                        and len(core_words2) > 0
                    ):
                        continue
                    ...
                    similarity = difflib.SequenceMatcher(
                        None, emp1["name"].lower(), emp2["name"].lower()
                    ).ratio()
    ...
    logger.info(
        f"  Filter efficiency: {(1 - similarity_calculations / comparisons) * 100:.1f}% filtered by quick checks"
        if comparisons > 0
        else ""
    )
...
def print_candidates(candidates: list[dict], limit: int | None = None):
    ...
    for i, candidate in enumerate(candidates, 1):
        ...
        print(
            f"               Location: {candidate['emp1_city']}, {candidate['emp1_state']}"
        )
        print(f"               Normalized: '{candidate['emp1_normalized']}'")
        print(f"   Employer 2: '{candidate['emp2_name']}'")
        print(
            f"               Location: {candidate['emp2_city']}, {candidate['emp2_state']}"
        )
...
def main():
    ...
    parser.add_argument("--output", type=Path, help="Output JSONL file path (optional)")
    parser.add_argument(
        "--limit",
        type=int,
    ...
    if args.format == "summary":
        ...
        if candidates:
            ...
            print(
                f"  Average similarity: {sum(c['similarity'] for c in candidates) / len(candidates):.3f}"
            )
