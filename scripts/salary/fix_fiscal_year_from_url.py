...
def get_fiscal_year_from_datasource_url(source_file: str) -> int | None:
    ...
    logger.debug(f"  Found DataSource {data_source.id}: URL={data_source.url}, local_file_path={data_source.local_file_path}")
    
    # Use shared function that handles all sophisticated strategies
    return get_fiscal_year_from_datasource(source_file, data_source, logger_instance=logger)
...
def fix_fiscal_years_for_artificial_files(dry_run: bool = True) -> dict:
    ...
    for file_info in salary_files:
        ...
        logger.info(f"  Current fiscal year distribution:")
        for year_info in current_years:
            logger.info(f"    FY {year_info['fiscal_year']}: {year_info['count']:,} records")
        ...
        records_to_fix = SalaryRecord.objects.filter(
        ...
        if fix_count == 0:
            logger.info(f"  ✓ All records already have correct fiscal year ({fiscal_year})")
            continue
        
        logger.info(f"  Found {fix_count:,} records to fix (should be FY {fiscal_year})")
        
        if dry_run:
            logger.info(f"  [DRY RUN] Would update {fix_count:,} records to fiscal_year={fiscal_year}")
        else:
            ...
            logger.info(f"  ✓ Updated {collector.count:,} records to fiscal_year={fiscal_year}")
    ...
    for file_info in worksite_files:
        ...
        # CRITICAL: Must use Q objects because .exclude() doesn't capture NULL values
        # In SQL, NULL != value evaluates to NULL (not TRUE), so exclude() misses NULL records
        records_to_fix = WorksiteRecord.objects.filter(
            source_file=source_file
        ).filter(
            Q(fiscal_year__isnull=True) | ~Q(fiscal_year=fiscal_year)
        )
