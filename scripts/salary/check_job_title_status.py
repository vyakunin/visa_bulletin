...
def main():
    ...
    print(
        f"Linked to JobTitle: {linked_records:,} ({linked_records * 100.0 / total_records:.1f}%)"
    )
    print(
        f"Not linked: {unlinked_records:,} ({unlinked_records * 100.0 / total_records:.1f}%)"
    )
    print()
    ...
    print(
        f"Clustered: {clustered_titles:,} ({clustered_titles * 100.0 / job_title_count:.1f}%)"
    )
    print(
        f"Not clustered: {unclustered_titles:,} ({unclustered_titles * 100.0 / job_title_count:.1f}%)"
    )
    print()
    ...
    print(
        f"{'✅' if clustered_titles > 0 else '❌'} Clustering: {'DONE' if clustered_titles > 0 else 'NOT DONE'}"
    )
    ...
    template_file = os.path.join(
        workspace_dir, "webapp", "templates", "webapp", "job_title_profile.html"
    )

    views_done = (
        os.path.exists(views_file)
        and "job_title_profile_view" in open(views_file).read()
        and os.path.exists(urls_file)
        and "job-title" in open(urls_file).read()
        and os.path.exists(template_file)
    )

    print(
        f"{'✅' if views_done else '❌'} Views integration: {'DONE' if views_done else 'NOT DONE'}"
    )
