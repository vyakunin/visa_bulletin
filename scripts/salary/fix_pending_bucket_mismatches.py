...
script_logger.log_call(
    args={}, context="Fixing pending bucket mismatch reviews that are clearly same"
)
...
for review in reviews:
    ...
    def clean_words(s):
        ...
        return {
            w
            for w in s.split()
            if w not in ["the", "a", "of", "and", "inc", "llc", "corp", "ltd"]
            and len(w) > 2
        }
    ...
    if words1 == words2 and len(words1) > 0:
        ...
        logger.info(
            f"  Will approve: '{review.employer1.name}' vs '{review.employer2.name}'"
        )
