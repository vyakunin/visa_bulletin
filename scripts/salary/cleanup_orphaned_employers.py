...
def find_orphaned_employers():
    ...
    employers_with_records = (
        SalaryRecord.objects.values("employer_id")
        .distinct()
...
def analyze_orphaned_employers():
    ...
    logger.info(
        f"Percentage: {(total_orphaned / total_employers * 100) if total_employers > 0 else 0:.1f}%"
    )
    logger.info("")
...
def cleanup_orphaned_employers(dry_run=False, delete=False, mark_inactive=False):
    ...
    if delete:
        logger.info(
            f"{'Would delete' if dry_run else 'Deleting'} {analysis['total']:,} orphaned employers..."
        )
        logger.info("")
        ...
        if not dry_run:
            ...
            for i in range(0, analysis["total"], batch_size):
                ...
                with transaction.atomic():
                    ...
                    deleted_count += count
                    logger.info(
                        f"  Deleted batch: {deleted_count:,} / {analysis['total']:,}"
                    )
            ...
            logger.info(
                f"[DRY-RUN] Would delete {analysis['total']:,} orphaned employers"
            )
        ...
        if not hasattr(Employer, "active") and not hasattr(Employer, "is_active"):
            logger.warning(
                "Employer model doesn't have an 'active' or 'is_active' field"
            )
        ...
        logger.info(
            f"{'Would mark as inactive' if dry_run else 'Marking as inactive'} {analysis['total']:,} orphaned employers..."
        )
        logger.info("")
        ...
        if not dry_run:
            ...
            for i in range(0, analysis["total"], batch_size):
                ...
                with transaction.atomic():
                    ...
                    updated_count += count
                    logger.info(
                        f"  Updated batch: {updated_count:,} / {analysis['total']:,}"
                    )
            ...
            logger.info(
                f"[DRY-RUN] Would mark {analysis['total']:,} orphaned employers as inactive"
            )
            return analysis["total"]
...
Examples:
  ...
  Mark orphaned employers as inactive (if model supports it):
    ...
    cleanup_orphaned_employers(
        dry_run=args.dry_run, delete=args.delete, mark_inactive=args.mark_inactive
    )
