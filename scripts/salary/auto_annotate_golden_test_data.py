...
def auto_annotate_case(test_case: dict, plugin_type: str) -> dict:
    ...
    try:
        ...
        return {"record_type": None, "expected_error": str(e)}
...
def main():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if test_case.get("record_type") is not None:
            ...
            if i % 100 == 0:
                print(
                    f"Progress: {i}/{len(test_cases)} (skipped {skipped_count} already annotated)"
                )
            continue
        ...
        try:
            ...
            if i % 50 == 0:
                print(
                    f"Progress: {i}/{len(test_cases)} (annotated {annotated_count}, skipped {skipped_count})"
                )
        except Exception as e:
            error_count += 1
