...
def get_files_with_missing_data(visa_program: VisaProgram | None = None) -> list[tuple[str, int, str]]:
    """
...
def build_case_number_index(filepath: Path, column_mappings: dict, target_case_numbers: set[str] | None = None) -> dict[str, dict]:
    """
    ...
    try:
        ...
        if target_case_numbers:
            ...
            logger.info(f"Building index for {len(target_case_numbers)} target case numbers (optimized mode)")
        else:
            logger.info(f"Building full case number index (may be slow for large files)")
        ...
        for record in records:
            ...
            if case_num:
                ...
                if target_normalized:
                    if normalized not in target_normalized and case_num_str not in target_original:
                ...
                if target_normalized and indexed_count >= len(target_case_numbers):
                    logger.info(f"Found all {len(target_case_numbers)} target case numbers, stopping early")
                    break
        
        logger.info(f"Indexed {indexed_count:,} case numbers from source file")
        return index
    except Exception as e:
        logger.error(f"Error building case number index from {filepath}: {e}", exc_info=True)
        return {}
...
def check_source_file_for_salary_data(
    ...
    try:
        ...
        if not wage_from or str(wage_from).strip() == '' or str(wage_from).strip() == '0':
        ...
        logger.error(f"Error checking case {case_number} in source file index: {e}", exc_info=True)
        return None
...
def check_file_has_salary_data(
    ...
    case_index = build_case_number_index(filepath, column_mappings, target_case_numbers=target_case_numbers)
    ...
    for case_number in sample_cases:
        result = check_source_file_for_salary_data(case_index, case_number, column_mappings)
        if result:
            if result.get('found'):
                found_count += 1
...
def update_records_from_file(
    ...
    logger.info(f"Found {total_salary_to_update:,} SalaryRecord and {total_worksite_to_update:,} WorksiteRecord records to update in {filepath.name}")
    
    if total_to_update == 0:
        return {'updated': 0, 'errors': 0, 'not_found': 0, 'dropped': 0, 'salary_updated': 0, 'worksite_updated': 0}
    ...
    if check_source_first:
        ...
        missing_salary_cases = list(existing_salary_records.values_list('case_number', flat=True)[:20])
        missing_worksite_cases = list(existing_worksite_records.values_list('case_number', flat=True)[:20]) if total_worksite_to_update > 0 else []
        ...
        logger.info(f"Source file check results:")
        ...
        if check_result['detailed_results']:
            ...
            for detail in check_result['detailed_results'][:5]:
                if detail['status'] == 'found':
                    logger.info(f"    {detail['case_number']}: FOUND - wage_from={detail.get('wage_from')}, unit={detail.get('wage_unit')}")
                else:
                    logger.info(f"    {detail['case_number']}: NOT FOUND - reason={detail.get('reason', 'unknown')}")
        
        if not check_result['has_data']:
            logger.warning(f"Source file does not appear to have salary data (checked {check_result['sample_checked']} samples)")
            ...
            logger.info(f"Source file has salary data (found in {check_result['found_in_source']}/{check_result['sample_checked']} samples)")
    ...
    try:
        if dry_run:
            ...
            if total_worksite_to_update > 0:
                logger.info(f"Re-importing {total_worksite_to_update:,} WorksiteRecord records...")
...
Examples:
  ...
  Limit for testing:
    ...
    parser.add_argument(
        '--file',
        type=str,
        help='Specific PERM file to process (filename only)'
    )
    ...
    if args.drop_unfixable:
        ...
        if not args.fix:
            logger.info(f"[DRY RUN] Would drop {salary_drop_count:,} SalaryRecord and {worksite_drop_count:,} WorksiteRecord records")
        else:
            with transaction.atomic():
                salary_deleted, _ = unfixable_salary.delete()
                worksite_deleted, _ = unfixable_worksite.delete()
                logger.info(f"Dropped {salary_deleted:,} SalaryRecord and {worksite_deleted:,} WorksiteRecord records")
