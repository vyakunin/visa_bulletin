...
def step1_check_worksite_status() -> dict:
    ...
    worksite_breakdown = missing_salary.aggregate(
        is_worksite_true=Sum(Case(When(is_worksite=True, then=1), default=0, output_field=IntegerField())),
        is_worksite_false=Sum(Case(When(is_worksite=False, then=1), default=0, output_field=IntegerField())),
        is_worksite_null=Sum(Case(When(is_worksite__isnull=True, then=1), default=0, output_field=IntegerField())),
    )
    ...
    results = {
        ...
        'worksite_true_pct': (worksite_true / total_missing * 100) if total_missing > 0 else 0,
        'worksite_false_pct': (worksite_false / total_missing * 100) if total_missing > 0 else 0,
    }

    print(f"\nTotal records with missing salary data: {total_missing:,}")
    print(f"  is_worksite=True:  {worksite_true:,} ({results['worksite_true_pct']:.1f}%)")
    print(f"  is_worksite=False: {worksite_false:,} ({results['worksite_false_pct']:.1f}%)")
...
def step2_analyze_by_source_file(limit: int = 20) -> list[dict]:
    ...
    source_file_stats = (
        ...
        .annotate(
            ...
            total_count=Count('id')  # This will be the count of missing records per file
        )
    ...
    for result in results:
        source_file_display = result['source_file'][:58] if len(result['source_file']) > 58 else result['source_file']
        print(f"{source_file_display:<60} {result['missing_count']:>11,} {result['total_in_file']:>11,} {result['missing_pct']:>11.1f}%")
...
def step3_check_by_visa_program() -> dict:
    ...
    total_by_program = (
        SalaryRecord.objects
        .filter(is_worksite=False)
        .values('visa_program')
        .annotate(total_count=Count('id'))
    )

    total_by_program_dict = {item['visa_program']: item['total_count'] for item in total_by_program}
    ...
    for stat in program_stats:
        ...
        program_label = next((p.label for p in VisaProgram if p.value == program_value), f"Unknown ({program_value})")
        missing_count = stat['missing_count']
        ...
        print(f"{program_label:<30} {missing_count:>11,} {total_count:>11,} {missing_pct:>11.1f}%")
...
def step4_sample_records(sample_size: int = 20) -> list[dict]:
    ...
    for i, record in enumerate(sample_records, 1):
        program_label = next((p.label for p in VisaProgram if p.value == record['visa_program']), f"Unknown ({record['visa_program']})")
        ...
        print(f"   wage_from: {record['wage_from']}, wage_unit: {record['wage_unit']}, wage_annual: {record['wage_annual']}")
        print(f"   is_worksite: {record['is_worksite']}")

    return sample_records


def generate_summary(step1_results: dict, step2_results: list, step3_results: dict, step4_results: list) -> str:
    ...
    summary_lines.append(f"  Worksite records: {step1_results['is_worksite_true']:,} ({step1_results['worksite_true_pct']:.1f}%)")
    summary_lines.append(f"  Non-worksite records: {step1_results['is_worksite_false']:,} ({step1_results['worksite_false_pct']:.1f}%)")

    if step1_results['is_worksite_false'] > step1_results['is_worksite_true']:
        summary_lines.append("  ⚠️  RECOMMENDATION: Most missing records are NOT worksite - investigate source files")
    else:
    ...
    if step2_results:
        ...
        summary_lines.append(f"    Missing: {top_file['missing_count']:,} ({top_file['missing_pct']:.1f}% of file)")
        if top_file['missing_pct'] > 50:
            summary_lines.append("  ⚠️  RECOMMENDATION: High percentage - investigate this source file")

    # Step 3 summary
    summary_lines.append("\nStep 3: Visa Program Analysis")
    for program_value, stats in step3_results.items():
        summary_lines.append(f"  {stats['program_label']}: {stats['missing_count']:,} missing ({stats['missing_pct']:.1f}%)")
        if stats['missing_pct'] > 20:
            summary_lines.append(f"    ⚠️  High percentage for {stats['program_label']}")

    # Step 4 summary
    summary_lines.append("\nStep 4: Sample Records")
    summary_lines.append(f"  Sampled {len(step4_results)} records for manual inspection")
    ...
    if step1_results['is_worksite_false'] > step1_results['is_worksite_true']:
        ...
        summary_lines.append("2. Manually inspect sample source files to check if salary data exists")
        summary_lines.append("3. Consider re-parsing affected source files if parsing issue found")
        summary_lines.append("4. Mark non-worksite records without salary as invalid if source data missing")
    else:
        summary_lines.append("1. Most missing records are worksite (expected)")
        summary_lines.append("2. Update validation to exclude worksite records from missing salary check")
        summary_lines.append("3. For non-worksite records with missing salary, investigate source files")
...
Examples:
  ...
  Sample more records:
    ...
    summary = generate_summary(step1_results, step2_results, step3_results, step4_results)
    print(summary)
