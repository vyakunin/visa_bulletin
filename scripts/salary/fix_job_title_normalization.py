...
def renormalize_job_titles(dry_run: bool = False):
    ...
    if dry_run:
        ...
        logger.info(
            "Would clear SalaryRecord.job_title_entity and recreate JobTitle records"
        )
    ...
    unique_titles = (
        SalaryRecord.objects.values("job_title")
    ...
    for i, title_data in enumerate(unique_titles, 1):
        ...
        if i % 10000 == 0:
            logger.info(
                f"Processed {i:,}/{len(unique_titles):,} ({i * 100.0 / len(unique_titles):.1f}%)"
            )
    ...
    logger.info(
        f"  Difference: {new_count - old_count:+,} ({(new_count - old_count) * 100.0 / old_count:+.1f}%)"
    )
