...
def find_affected_source_files() -> set[str]:
    ...
    try:
        salary_files = set(
            ...
            .filter(case_number__startswith='I-200')
        ...
        if source_file:
            try:
                salary_count = SalaryRecord.objects.filter(
                    ...
                    case_number__startswith='I-200'
                ).count()
                worksite_count = WorksiteRecord.objects.filter(
                    source_file=source_file,
                    case_number__startswith='I-200'
                ).count()
                logger.info(f"  {source_file}: {salary_count:,} in SalaryRecord, {worksite_count:,} in WorksiteRecord")
            except Exception as e:
...
def delete_i200_records_for_file(source_file: str, dry_run: bool = False, skip_if_in_worksite: bool = True) -> tuple[int, int, bool]:
    """
    ...
    """
    logger.info(f"{'[DRY RUN] Would delete' if dry_run else 'Deleting'} I-200 records from {source_file}...")
    ...
    if skip_if_in_worksite and worksite_count > 0:
        logger.info(f"  Skipping {source_file} - already has {worksite_count:,} I-200 records in WorksiteRecord (correctly migrated)")
    ...
    logger.info(f"  Found {worksite_count:,} I-200 records in WorksiteRecord (not deleted)")
...
def find_or_create_data_source(source_file: str) -> DataSource | None:
    ...
    logger.warning(f"  You may need to discover sources first: bazel run //scripts/ingest:run_pipeline -- discover --domain dol")
    return None
...
def main():
    ...
    if not args.skip_delete:
        ...
        logger.info(f"Skipped {total_skipped} files (already correctly migrated to WorksiteRecord)")
    ...
    for source in all_lca_sources:
        ...
        try:
            ...
            filename_match = any(
                filename.lower().replace('.xlsx', '').replace('.csv', '').replace(' ', '') in url_lower.replace(' ', '')
    ...
    if data_sources:
        logger.info("Files that need re-import (I-200 records were deleted from SalaryRecord):")
        for source in data_sources:
            logger.info(f"  bazel run //scripts/ingest:run_pipeline -- run --source-id {source.id}")
        ...
        logger.info(f"    bazel run //scripts/ingest:run_pipeline -- run --source-id $id")
        logger.info(f"  done")
    else:
        logger.warning("No matching DataSource entries found.")
        logger.info("You may need to discover sources first:")
        logger.info("  bazel run //scripts/ingest:run_pipeline -- discover --domain dol")
        logger.info("")
