...
def find_or_create_data_source(source_file: str) -> DataSource | None:
    ...
    logger.warning("  You may need to discover sources first: bazel run //scripts/ingest:run_pipeline -- discover --domain dol")
    return None
...
def main():
    ...
    if data_sources:
        ...
        logger.info("    bazel run //scripts/ingest:run_pipeline -- run --source-id $id")
        logger.info("  done")
    else:
