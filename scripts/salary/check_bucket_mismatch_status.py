...
print(f"Bucket mismatch reviews in database:")
...
for review in bucket_mismatch_reviews.filter(status='approved')[:5]:
    ...
    print(f"    {review.employer1.name_normalized} vs {review.employer2.name_normalized}")

if pending > 0:
    print(f"\nPending reviews (need manual review):")
    for review in bucket_mismatch_reviews.filter(status='pending')[:10]:
        print(f"  '{review.employer1.name}' vs '{review.employer2.name}'")
        print(f"    {review.employer1.name_normalized} vs {review.employer2.name_normalized} | sim: {review.similarity_score:.3f}")
