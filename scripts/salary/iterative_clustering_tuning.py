...
def call_ollama(
    prompt: str, model: str = "llama3.2:3b", max_retries: int = 3
) -> str | None:
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning(
                "Ollama not found. Install from https://ollama.ai/ or use --skip-llm"
            )
            return None
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.debug(
                        f"Ollama call failed, retrying in {wait_time}s (attempt {attempt + 1}/{max_retries})"
                    )
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.debug(
                        f"Ollama call timed out, retrying with longer timeout in {wait_time}s"
                    )
                    time.sleep(wait_time)
                else:
                    logger.warning(
                        f"Ollama call timed out after {max_retries} attempts"
                    )
        return None
...
def evaluate_samples(
    auto_sample: list, queue_sample: list, skip_llm: bool = False
    ...
    for pair in auto_sample:
        ...
        if is_same:
            ...
            logger.warning(
                f"✗ FP: {pair['emp1_name']} <-> {pair['emp2_name']} (similarity: {pair['similarity']:.3f})"
            )
    ...
    auto_recall = (
        auto_tp / max(auto_tp + queue_tp, 1) if (auto_tp + queue_tp) > 0 else 0.0
    )
...
def suggest_threshold_adjustment(
    metrics: dict, current_threshold: float, target_precision: float = 0.99
) -> float:
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Iterative clustering tuning for 99% precision"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Sample size per category (default: 500 for 99% precision measurement)",
    )
    parser.add_argument(
        "--initial-threshold", type=float, default=0.95, help="Initial threshold"
    )
    parser.add_argument(
        "--target-precision",
        type=float,
        default=0.99,
        help="Target precision (default: 0.99)",
    )
    parser.add_argument(
        "--output-dir", default="/tmp/clustering_tuning", help="Output directory"
    )
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument(
        "--skip-llm",
        action="store_true",
        help="Skip LLM validation, use heuristics instead",
    )
    parser.add_argument(
        "--max-runtime",
        type=int,
        default=600,
        help="Maximum runtime in seconds (default: 600 = 10 min)",
    )
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        if elapsed > args.max_runtime:
            logger.warning(
                f"Maximum runtime ({args.max_runtime}s) reached. Stopping after {iteration - 1} iterations."
            )
            break
        logger.info(f"\n{'=' * 60}")
        ...
        # Sample pairs
        auto_sample = (
            random.sample(auto_clustered, min(args.sample_size, len(auto_clustered)))
            if auto_clustered
            else []
        )
        queue_sample = (
            random.sample(queued, min(args.sample_size, len(queued))) if queued else []
        )
        ...
        print(
            f"  Precision: {metrics['overall']['precision']:.1%} (target: {args.target_precision:.1%})"
        )
        print(f"  Recall: {metrics['overall']['recall']:.1%}")
        print(f"  F1 Score: {metrics['overall']['f1_score']:.3f}")
        print(
            f"  Auto-clustered: {metrics['auto_clustered']['true_positives']}/{metrics['auto_clustered']['total_evaluated']} correct"
        )
        ...
        if false_positives:
            ...
            for fp in false_positives[:5]:
                print(
                    f"    - {fp['emp1_name']} <-> {fp['emp2_name']} (similarity: {fp['similarity']:.3f})"
                )
        ...
        if false_negatives:
            ...
            for fn in false_negatives[:5]:
                print(
                    f"    - {fn['emp1_name']} <-> {fn['emp2_name']} (similarity: {fn['similarity']:.3f})"
                )
        ...
        if abs(precision - args.target_precision) <= 0.005 and iteration >= 2:
            logger.info(
                f"✓ Target precision ({args.target_precision:.1%}) achieved after {iteration} iterations!"
            )
            logger.info(
                f"  Precision: {precision:.1%} (target: {args.target_precision:.1%}, tolerance: ±0.5%)"
            )
            break
        elif precision >= args.target_precision:
            logger.info(
                f"✓ Target precision ({args.target_precision:.1%}) achieved, but continuing to validate..."
            )

        # Suggest new threshold
        new_threshold = suggest_threshold_adjustment(
            metrics, current_threshold, target_precision=args.target_precision
        )
        logger.info(
            f"Suggested new threshold: {new_threshold:.3f} (current: {current_threshold:.3f})"
        )
        ...
        if iteration < args.iterations:
            ...
            if remaining < wait_time + 60:  # Need at least 60s for next iteration
                logger.warning(
                    f"Not enough time remaining ({remaining:.0f}s). Stopping early."
                )
                break
    ...
    if iterations:
        ...
        print(
            f"Final precision: {final['metrics']['overall']['precision']:.1%} (target: {args.target_precision:.1%})"
        )
