...
import lib.ingest.plugins  # Trigger registration
...
def test_plugins():
    ...
    for domain, source_type, plugin in plugins:
        print(f"Domain: {domain} | Source: {source_type} | Class: {plugin.__class__.__name__}")
    ...
    if inventory_file.exists():
        ...
        for record in uscis.parse(inventory_file, run):
            ...
            if fact:
                print(f"  Generated Fact: {fact.dimensions['country']} {fact.dimensions['visa_class']} {fact.value}")
                count += 1
    ...
    if perm_file.exists():
        ...
        for record in perm.parse(perm_file, run):
            ...
            if fact:
                print(f"  Generated Fact: {fact.dimensions['country']} {fact.dimensions['visa_class']} {fact.dimensions['status']} val={fact.value}")
                count += 1
        print(f"  Total Facts: {count}")
    else:
