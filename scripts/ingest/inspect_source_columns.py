...
def _validate_parsing(
            ...
            try:
                ...
                if model_instance:
                    ...
                    samples.append(
                        {
                            ...
                            "employer_name": getattr(
                                model_instance, "employer_name", None
                            )
                            if hasattr(model_instance, "employer_name")
                            else "N/A",
                    ...
                    if validation_errors:
                        errors.extend(
                            [f"Row {idx + 1}: {err}" for err in validation_errors]
                        )
                else:
                    warnings.append(
                        f"Row {idx + 1}: transform() returned None (rejected)"
                    )
    ...
    if samples:
        ...
        for sample in samples[:3]:  # Show first 3
            ...
            logger.info(
                "      Employer: %s",
                sample["employer_name"][:50] if sample["employer_name"] else "N/A",
            )
            logger.info(
                "      Job: %s",
                sample["job_title"][:50] if sample["job_title"] else "N/A",
            )
            if sample["validation_errors"]:
                logger.info(
                    "      ⚠️  Validation errors: %s",
                    ", ".join(sample["validation_errors"]),
                )
...
def main() -> None:
    ...
    exclude_terms = [
        term.strip() for term in args.exclude_terms.split(",") if term.strip()
    ]
    ...
    if args.extract_row:
        ...
        for file_path in files:
            ...
            for i, row in enumerate(
                ws.iter_rows(min_row=target_row, max_row=target_row, values_only=True),
                start=target_row,
            ):
                found = True
                logger.info("\nRow %d values:", i)
    ...
    for file_path in files:
        ...
        logger.info(
            "Total records in %s: %s", file_path.name, f"{file_stats['row_count']:,}"
        )
        logger.info("")
