...
One-time steps to move from the current situation (everything on `main`, scp patches, drift) to the branch-based workflow. Do these in order.
...
- [ ] Local WIP committed to `wip/pre-cleanup` and pushed
- [ ] `prod` branch created from production server state, pushed
- [ ] `staging` branch created from staging server state, pushed
- [ ] Production server tracking `prod` branch
- [ ] Staging server tracking `staging` branch
- [ ] GitHub Actions triggers updated (staging/prod, not main)
- [ ] Volume mount hack removed from production server
- [ ] `deploy.sh` deprecated or rewritten
- [ ] Both servers verified healthy after changes
- [ ] Team (future agents) can discover the workflow via `.cursor/rules/branching.mdc`
