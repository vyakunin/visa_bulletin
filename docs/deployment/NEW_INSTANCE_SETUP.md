...
# Check index count
ssh prod_2Gb_vm \
  "sudo -u postgres psql -d visa_bulletin -t -c \
...
```bash
psql -d visa_bulletin -c "SELECT pid, usename, datname, state, wait_event, query FROM pg_stat_activity WHERE datname = 'visa_bulletin' ORDER BY query_start;"
```
...
```bash
psql -d visa_bulletin -c "SELECT pg_terminate_backend(PID);"
```

**Check database schema:**
```bash
psql -d visa_bulletin -c "\d+ salary_record"
```
...
- PostgreSQL connection variables: **DB_NAME=visa_bulletin** (single DB per instance; no blue/green), DB_USER, DB_PASSWORD, DB_HOST, DB_PORT
...
- Create **`visa_bulletin`** database and user (this is the only DB name used; no visa_bulletin_blue/green).
- Verify authentication.

**Existing prod/staging:** To confirm both instances use `DB_NAME=visa_bulletin`, see **deployment/README.md** → “Database name (prod and staging)” (verify and fix commands).
...
- **Issue:** Default `/opt/visa_bulletin/docker-compose.yml` uses SQLite and legacy data-refresh loop.
  - **Fix:** Use `deployment/docker-compose.yml` (single stack).
