...
from .enums.visa_program import VisaProgram, WageUnit, CaseStatus
...
class Employer(models.Model):
    ...
    total_lca_count = models.IntegerField(
        default=0,
        help_text="Total H-1B LCA applications"
    )
    
    total_perm_count = models.IntegerField(
        default=0,
        help_text="Total PERM applications"
    )
    ...
    def normalize_name(name: str) -> str:
        ...
        normalized = re.sub(r'\s+\d+\s+', ' ', normalized)  # Remove numbers with spaces on both sides
        ...
        for suffix in [', inc.', ', inc', ' inc.', ' inc', ', llc', ' llc', 
                       ', corp.', ', corp', ' corp.', ' corp', ', ltd.', 
                       ', ltd', ' ltd.', ' ltd', ', l.p.', ' l.p.',
                       ', co.', ', co', ' co.', ' co', ', incorporated', ' incorporated']:
        ...
        critical_generic_words = {
            'capital', 'holdings', 'technology', 'tech', 'consulting', 'consultants',
            'partners', 'associates', 'management', 'group', 'groups',
            'usa', 'us',  # Geographic indicators can distinguish entities (e.g., "Roca USA" vs "Roca")
        }
        ...
        if len(non_generic_words) >= 2:
            ...
            filtered_words = non_generic_words + distinguishing_generic + critical_generic
        else:
...
class SalaryRecord(models.Model):
    ...
    employer_name = models.CharField(
        max_length=255,
        db_index=True,
        help_text="Employer name from DOL data"
    )
