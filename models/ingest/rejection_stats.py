...
class IngestRejectionStats(models.Model):
    ...
    reason = models.CharField(
        max_length=50, choices=RejectionReason.choices, help_text="Reason for rejection"
    )
    count = models.IntegerField(
        default=0, help_text="Number of records rejected for this reason"
    )
    sample_case_numbers = models.JSONField(
        default=list, help_text="Sample case numbers (up to 10) for investigation"
    )
    ...
    def __str__(self):
        return (
            f"Run {self.run_id}: {self.get_reason_display()} ({self.count:,} records)"
        )
