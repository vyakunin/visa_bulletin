...
class EmploymentPreference(models.TextChoices):
    ...
    def normalize_for_display(cls, visa_class: str) -> str:
        ...
        if "5" in clean or "eb-5" in clean_lower:
            ...
            if (
                "high unemployment" in clean_lower
                or "(nh" in clean_lower
                or "rh" in clean_lower
                or "nh," in clean_lower
            ):
                return "EB-5: High Unemployment (10%)"
            if (
                "infrastructure" in clean_lower
                or "(ri" in clean_lower
                or "ri)" in clean_lower
            ):
                return "EB-5: Infrastructure (2%)"
            if (
                "rural" in clean_lower
                or "(nr" in clean_lower
                or "rr" in clean_lower
                or "nr," in clean_lower
            ):
            ...
            if (
                "targeted" in clean_lower
                or "regional" in clean_lower
                or "employ-ment" in clean_lower
                or "employmentareas" in clean_lower
            ):
