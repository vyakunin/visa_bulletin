...
def convert_visa_program_to_int(apps, schema_editor):
    ...
    for string_value, int_value in visa_program_mapping.items():
        SalaryRecord.objects.filter(visa_program=string_value).update(
            visa_program=int_value
        )
...
def convert_case_status_to_int(apps, schema_editor):
    ...
    for string_value, int_value in case_status_mapping.items():
        SalaryRecord.objects.filter(case_status=string_value).update(
            case_status=int_value
        )
...
def reverse_visa_program_to_string(apps, schema_editor):
    ...
    for int_value, string_value in int_to_string.items():
        SalaryRecord.objects.filter(visa_program=int_value).update(
            visa_program=string_value
        )
...
def reverse_case_status_to_string(apps, schema_editor):
    ...
    for int_value, string_value in int_to_string.items():
        SalaryRecord.objects.filter(case_status=int_value).update(
            case_status=string_value
        )
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        (
            "models",
            "0005b_create_bulletin_models",
        ),  # Bulletin and VisaCutoffDate must exist first
    ]

    operations = [
        # Convert data BEFORE changing field types
        migrations.RunPython(
            convert_visa_program_to_int, reverse_visa_program_to_string
        ),
