...
- **`curl` to `/employer/*` and `/job-title/*` gets Cloudflare's challenge page, not the site — and the challenge page carries `<meta name="robots" content="noindex,nofollow">`.** The managed-challenge rule on the profile surfaces answers **403** with `<title>Just a moment...</title>` to curl regardless of `-A`, because it fingerprints the TLS handshake, not the User-Agent. So a curl-based scrape of those paths measures the challenge, not the page: sampling sitemap URLs this way reports the whole profile long tail as noindexed, which is false. Verified bots (Googlebot, bingbot) are exempt and reach origin normally — 22.3k `/job-title/` + 21.2k `/employer/` hits in a 24h nginx window — so this is scraper mitigation working, not a regression. **Always check the status code and `<title>` before believing any parsed field**; a 403 titled "Just a moment..." means you read Cloudflare. To read the real HTML, go to the origin and bypass the edge entirely:
  ```bash
  uv run scripts/origin_check.py body   /employer/<slug>/      # the real HTML
  uv run scripts/origin_check.py status / /salaries/ /employers/   # code per path, exit 1 on any mismatch
  ```
  `origin_check.py` owns this fetch (`scripts/README.md` § Origin route checks). Do **not**
  hand-assemble the `ssh homeserver "docker exec vb_nginx sh -c \"wget --header=...\""` form:
  it nests three levels of shell quoting, and a path holding `?`/`&` loses its query string
  while still returning 200. (`vb_web` ships no curl/wget — the fetch runs from `vb_nginx`,
  per `deployment.md`.)
- **`origin_check.py body` renders `http://` canonicals — that is the PROBE, not a defect.** The origin fetch is plain HTTP inside the Docker network, so `request.build_absolute_uri` yields `http://visa-bulletin.us/...` and the page's `<link rel="canonical">` comes back on `http`. Public pages served through Cloudflare are correct self-referencing `https` — verified 2026-08-30 across `/`, `/predictions/<month>-<year>/`, `/employment-based/<country>/`, `/priority-date/<eb>/<country>/`, `/family-sponsored/<country>/` and `/analysis/<slug>/`. It reads exactly like the stale-Redis wrong-canonical bug `deployment.md` warns about, and the two are told apart the same way: **confirm any canonical/absolute-URL claim against the PUBLIC URL through Cloudflare before calling it a bug.** Anything the page derives from the request — canonical, `og:url`, `twitter:url`, JSON-LD `url` — carries the same caveat; the page's own content does not. (Cost a detour on a Bing Webmaster "same canonical URL for a large number of pages" warning that did not reproduce publicly.)
