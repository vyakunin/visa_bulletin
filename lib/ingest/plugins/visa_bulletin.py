...
from lib.parsing.bulletin.parser import extract_tables, extract_tables_modern, extract_tables_legacy
...
class VisaBulletinPlugin(DataSourcePlugin):
    ...
    def parse(self, filepath: Path, run: IngestRun) -> Iterator[dict]:
        ...
        if not pub_date:
            logger.warning(f"[Run {run.id}] No publication date extracted, using today's date")
        ...
        for table in tables:
            ...
            for cutoff_data in cutoff_data_list:
                yield {
                    ...
                    '_publication_date': publication_date.date() if publication_date else None,
    ...
    def validate_post_ingest(self, run: IngestRun) -> ValidationResult:
        ...
        if not publication_date:
            ...
            from datetime import datetime
            filename = Path(run.checkpoint.get('filepath', '')).name if run.checkpoint.get('filepath') else None
            if filename:
                try:
                    date_str = filename.replace('visa-bulletin-for-', '').replace('.html', '')
        ...
        if publication_date:
            ...
            bulletin = Bulletin.objects.filter(publication_date=publication_date).first()
            if bulletin:
                ...
                if record_count == 0:
                    errors.append(f"No cutoff dates created for bulletin {publication_date} - expected data but got none")
                
                if record_count > 0:
                    # Check for required fields
                    missing_category = cutoff_dates.filter(visa_category__isnull=True).count()
                    missing_class = cutoff_dates.filter(visa_class__isnull=True).exclude(visa_class='').count()
                    missing_country = cutoff_dates.filter(country__isnull=True).count()
                    
                    if missing_category > 0:
                        errors.append(f"{missing_category} cutoff dates missing visa_category")
                    if missing_class > 0:
                        errors.append(f"{missing_class} cutoff dates missing visa_class")
                    ...
                    invalid_category = cutoff_dates.exclude(visa_category__in=valid_categories).count()
                    invalid_action = cutoff_dates.exclude(action_type__in=valid_actions).count()
                    invalid_country = cutoff_dates.exclude(country__in=valid_countries).count()
                    
                    if invalid_category > 0:
                        errors.append(f"{invalid_category} cutoff dates have invalid visa_category")
                    if invalid_action > 0:
                        errors.append(f"{invalid_action} cutoff dates have invalid action_type")
                    if invalid_country > 0:
                        errors.append(f"{invalid_country} cutoff dates have invalid country")
                    
                    # Distribution checks
                    categories = cutoff_dates.values_list('visa_category', flat=True).distinct()
                    classes = cutoff_dates.values_list('visa_class', flat=True).distinct()
                    countries = cutoff_dates.values_list('country', flat=True).distinct()
                    
                    details['categories'] = list(categories)
                    details['classes'] = list(classes)[:20]  # Limit to avoid huge details
                    ...
                    if len(countries) < 5:
                        warnings.append(f"Unusually few countries ({len(countries)}) - expected more")
            else:
                errors.append(f"Bulletin not found for publication date {publication_date}")
        else:
