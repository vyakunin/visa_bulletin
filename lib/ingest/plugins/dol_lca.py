...
class H1BSalaryDataSourcePlugin(DataSourcePlugin):
    ...
    def discover_sources(self) -> list[SourceInfo]:
        ...
        try:
            ...
            logger.info(
                f"Discovered {len(sources)} LCA/worksite data sources ({len(lca_matches)} LCA, {len(worksite_matches)} worksite)"
            )
    ...
    def _parse_excel_streaming(self, filepath: Path, run: IngestRun) -> Iterator[dict]:
        ...
        start_row = (
            run.checkpoint.get("last_row", 0) + 2
    ...
    def transform(self, record: dict) -> SalaryRecord | WorksiteRecord | None:
        ...
        if (
            ...
            if self._rejection_tracker:
                self._rejection_tracker.record_rejection(
                    "whitelist_filtered", case_number_value
                )
            return None
        ...
        if is_worksite_file or case_number_value.startswith("I-200"):
            ...
            if self.skip_worksite:
                if self._rejection_tracker:
                    self._rejection_tracker.record_rejection(
                        "worksite_skipped", case_number_value
                    )
                return None
    ...
    def _transform_to_salary_record(
        self, record: dict, column_mappings: dict
    ) -> SalaryRecord | None:
        ...
        if not employer_name_raw or employer_name_raw.strip() == "":
            ...
            if self._rejection_tracker:
                self._rejection_tracker.record_rejection(
                    "missing_employer_name", case_number_value
                )
            return None
        ...
        if employer_name_raw.strip().lower() == "unknown":
            ...
            if self._rejection_tracker:
                self._rejection_tracker.record_rejection(
                    "unknown_employer_name", case_number_value
                )
            return None
        ...
        if not job_title_raw or job_title_raw.strip() == "":
            ...
            if self._rejection_tracker:
                self._rejection_tracker.record_rejection(
                    "missing_job_title", case_number_value
                )
            return None
        ...
        if job_title_raw.strip().lower() == "unknown":
            ...
            if self._rejection_tracker:
                self._rejection_tracker.record_rejection(
                    "unknown_job_title", case_number_value
                )
            return None
        ...
        if not wage_from and not wage_annual:
            ...
            if self._rejection_tracker:
                self._rejection_tracker.record_rejection(
                    "missing_wage_data", case_number_value
                )
            return None
        ...
        if wage_annual is not None:
            ...
            if not is_valid:
                ...
                if self._rejection_tracker:
                    self._rejection_tracker.record_rejection(
                        "invalid_wage_range", case_number_value
                    )
                return None
    ...
    def _transform_to_worksite_record(
        self, record: dict, column_mappings: dict
    ) -> WorksiteRecord | None:
        ...
        if wage_annual is not None:
            ...
            if not is_valid:
                logger.debug(
                    f"Skipping worksite record {case_number_value}: wage_annual outside valid range"
                )
                return None
        ...
        worksite_state_raw = (
            get_column_value(record, column_mappings["worksite_state"]) or ""
        )
        ...
        worksite_zip = (
            get_column_value(record, WORKSITE_COLUMN_MAPPINGS.get("worksite_zip", []))
            or ""
        )
    ...
    def validate_post_ingest(self, run: IngestRun) -> ValidationResult:
        ...
        worksite_errors = (
            worksite_result.errors.copy() if worksite_result.errors else []
        )
        ...
        salary_count = (
            salary_result.details.get("records_created", 0)
            if salary_result.details
            else 0
        )
        worksite_count = (
            worksite_result.details.get("records_created", 0)
            if worksite_result.details
            else 0
        )
        ...
        if total_records == 0:
            source_file = (
                salary_result.details.get("source_file")
                if salary_result.details
                else None
            )
            errors.append(
