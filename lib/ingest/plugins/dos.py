...
from lib.utils.http_utils import fetch_page, download_file
...
class DosIssuancePlugin(DataSourcePlugin):
        ...
        with pdfplumber.open(filepath) as pdf:
            for page in pdf.pages:
                ...
                for row in table:
                    ...
                    # Skip header rows
                    if "Foreign State" in row[0] or "Visa Class" in row[0] or "Chargeability" in row[0]:
                        continue
                    ...
                    if not country or country == "Grand Total" or "TOTAL" in country.upper():
                        continue
                    ...
                    n_country_val = int(n_country) if hasattr(n_country, 'value') else n_country
                    
                    fact_key = (month_str, year_str, n_country_val, visa_class.upper().strip())
    ...
    def transform(self, record: dict) -> RawFactsLedger | None:
        ...
        if v_upper in ["E1", "E11", "E12", "E13"]: v_cat = "1st"
        elif v_upper in ["E2", "E21", "E22", "E23"]: v_cat = "2nd"
        elif v_upper in ["E3", "E31", "E32", "E34", "E35"]: v_cat = "3rd"
        elif v_upper in ["EW", "EW3", "EW4", "EW5"]: v_cat = "3rd" # Other Workers -> 3rd
        elif v_upper.startswith("S") or v_upper.startswith("R") or v_upper == "E4": v_cat = "4th"
        elif v_upper.startswith("C5") or v_upper.startswith("T5") or v_upper.startswith("I5") or v_upper.startswith("R5") or v_upper == "E5": v_cat = "5th"
        
        # If unknown, try simplified defaults
        if v_cat == "Unknown":
            if "1" in v_upper: v_cat = "1st"
            elif "2" in v_upper: v_cat = "2nd"
            elif "3" in v_upper: v_cat = "3rd"
            elif "4" in v_upper: v_cat = "4th"
            elif "5" in v_upper: v_cat = "5th"
