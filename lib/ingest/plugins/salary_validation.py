...
from lib.parsing.salary.wage_unit_correction import MIN_ANNUAL, MAX_ANNUAL
...
HIGH_WAGE_REVIEW_THRESHOLD_PCT = 1.0  # Flag if >1% of records have wages outside valid range


def _get_records_for_run(run: IngestRun, visa_program: VisaProgram, model_class) -> tuple[models.QuerySet, str | None]:
...
def _validate_required_fields(records: models.QuerySet, model_class, errors: list[str]) -> None:
    ...
    if hasattr(model_class, 'employer_name'):
        ...
        if missing_employer > 0:
            errors.append(f"{missing_employer} records missing employer_name (required field)")
...
def _validate_wage_ranges(records: models.QuerySet, warnings: list[str]) -> None:
    ...
    if extremely_high_count > 0:
        ...
        examples = list(extremely_high.values_list('case_number', 'wage_annual', 'wage_unit', 'job_title')[:5])
        example_str = ", ".join([f"{c}: ${float(w):,.0f} ({u})" for c, w, u, j in examples])
    ...
    if extremely_low_count > 0:
        ...
        examples = list(extremely_low.values_list('case_number', 'wage_annual', 'wage_unit', 'job_title')[:5])
        example_str = ", ".join([f"{c}: ${float(w):,.0f} ({u})" for c, w, u, j in examples])
...
def _check_null_rates(records: models.QuerySet, model_class, record_count: int, warnings: list[str]) -> dict:
    ...
    for field in fields_to_check:
        if hasattr(model_class, field):
            if field == 'worksite_state':
                non_null_count = records.filter(**{f'{field}__isnull': False}).exclude(**{field: ''}).count()
            else:
                non_null_count = records.filter(**{f'{field}__isnull': False}).count()
            
            null_pct = (record_count - non_null_count) / record_count * 100 if record_count > 0 else 0
...
def _check_fiscal_year_distribution(records: models.QuerySet, warnings: list[str]) -> list:
    ...
    if len(fiscal_years_list) == 0:
        ...
        warnings.append(f"Unusual number of fiscal years in single file: {len(fiscal_years_list)}")
    
    return fiscal_years_list


def _validate_state_codes(records: models.QuerySet, model_class, warnings: list[str]) -> None:
    """Validate state codes if worksite_state field exists."""
    if not hasattr(model_class, 'worksite_state'):
        return
    
    invalid_states = records.filter(
...
def validate_salary_records_post_ingest(
    ...
    details['null_rates'] = _check_null_rates(records, model_class, record_count, warnings)
