...
from models.ingest.enums import IngestStage, IngestStatus, SourceType
...
from .versioning import activate_version, create_version
...
def _identify_new_and_existing(
    records: list, batch_size: int = 10000
) -> tuple[list, dict]:
    """
    ...
    for i in range(0, len(case_numbers), batch_size):
        ...
        existing = model_class.objects.filter(case_number__in=batch_cases).only(
            *only_fields
        )
...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        if self.update_mode:
            if not self.update_fields:
                raise ValueError(
                    "update_fields must be specified when update_mode=True"
                )
    ...
    def run(
        ...
        if filepath:
            logger.info(
                f"[Run {run.id}] Estimating row count from {filepath.suffix.upper()} file..."
            )
            total_expected_records = count_file_rows(filepath, logger_instance=logger)
            if total_expected_records is not None:
                logger.info(
                    f"[Run {run.id}] Estimated {total_expected_records:,} data rows"
                )
                ...
                logger.info(
                    f"[Run {run.id}] Could not estimate total records (will proceed without ETA)"
                )
        else:
            logger.info(
                f"[Run {run.id}] Could not estimate total records (will proceed without ETA)"
        ...
        try:
            ...
            with transaction.atomic():
                ...
                if run.status == IngestStatus.RUNNING:
                    raise RuntimeError(
                        f"Run {run.id} is already running in another process"
                    )
            ...
            if run.stage in [IngestStage.PENDING, IngestStage.DOWNLOADING]:
                ...
                if not filepath or not filepath.exists():
                    raise FileNotFoundError(
                        f"Checkpoint filepath not found: {filepath}"
                    )
            ...
            if not self.update_mode:
                ...
                logger.info(
                    f"[Run {run.id}] Update mode: Skipping versioning (updates modify existing records)"
                )
            ...
            if not self.update_mode:
                logger.info(
                    f"[Run {run.id}] Pipeline completed successfully (version: {version_tag})"
                )
            else:
                logger.info(
                    f"[Run {run.id}] Pipeline completed successfully (update mode)"
                )
    ...
    def _get_or_create_run(self, source: DataSource, resume: bool) -> IngestRun:
        ...
        if resume:
            ...
            with transaction.atomic():
                existing = (
                    IngestRun.objects.select_for_update()
                    .filter(
                        source=source,
                ...
                if existing:
                    ...
                    if existing.status == IngestStatus.RUNNING:
                        ...
                        logger.warning(
                            f"[Run {existing.id}] Another process is running, creating new run"
                        )
                    else:
                        logger.info(
                            f"[Run {existing.id}] Resuming existing run from stage: {existing.stage}"
                        )
                        return existing
    ...
    def _download_stage(
        self, plugin: DataSourcePlugin, source: DataSource, run: IngestRun
    ) -> Path:
        ...
        if source_file_date:
            ...
            logger.debug(
                f"[Run {run.id}] Extracted source file date: {source_file_date}"
            )
        else:
            logger.warning(
                f"[Run {run.id}] Could not extract source file date from {filepath}"
            )
        ...
        logger.info(
            f"[Run {run.id}] Download completed in {stage_duration:.2f}s: {filepath}"
        )
        return filepath

    def _parse_stage(
        self, plugin: DataSourcePlugin, filepath: Path, run: IngestRun
    ) -> Iterator[dict]:
        ...
        run.checkpoint["stage"] = (
            IngestStage.TRANSFORMING.value
        )  # Store integer in JSON
        run.save()

        stage_duration = time.time() - stage_start
        self.stage_timings["parse"] = stage_duration
        logger.info(
            f"[Run {run.id}] Parse stage ready (duration: {stage_duration:.2f}s)"
        )
        return records

    def _transform_stage(
        self, plugin: DataSourcePlugin, records: Iterator[dict], run: IngestRun
    ) -> Iterator:
        ...
        if skip_records > 0:
            logger.warning(
                f"[Run {run.id}] DEBUG MODE: Skipping first {skip_records:,} records"
            )

        transform_error_count = 0
        records_processed_count = 0
        last_record_num = (
            0  # Always set; for-loop variable may be unbound if records is empty
        )
        ...
        for record_num, record in enumerate(records, start=1):
            ...
            if record_num % 1000 == 0:
                transform_rate_logger.log(
                    f"[Run {run.id}] Transforming record {record_num:,}..."
                )
            ...
            def handle_recoverable_error(e: Exception) -> None:
                ...
                if transform_error_count <= 5:  # Log first 5 errors in detail
                    logger.error(
                        f"[Run {run.id}] Transform failed for record {record_num}: {e}",
                        exc_info=True,
                    )
                elif transform_error_count == 6:
                    logger.error(
                        f"[Run {run.id}] Transform errors continuing, suppressing detailed logs..."
                    )
            ...
            if model:
                ...
                if records_processed_count % 10000 == 0:
                    logger.debug(
                        f"[Run {run.id}] Transformed {records_processed_count:,} records"
                    )
    ...
    def _load_to_db_stage(self, models: Iterator, run: IngestRun):
        ...
        if self.update_mode:
            logger.info(
                f"[Run {run.id}] Stage: UPDATING DATABASE (update_mode=True, fields={self.update_fields})"
            )
        else:
        ...
        if self.adaptive_batch:
            ...
            try:
                ...
                logger.info(
                    f"[Run {run.id}] Counting existing {model_class.__name__} records to determine batch size..."
                )
                count_start = time.time()
                current_count = model_class.objects.count()
                count_duration = time.time() - count_start
                logger.info(
                    f"[Run {run.id}] Found {current_count:,} existing {model_class.__name__} records (count took {count_duration:.2f}s)"
                )
                ...
                if self.use_copy:
                    self.batch_size = max(self.batch_size, 10000)
                logger.info(
                    f"[Run {run.id}] Adaptive batch size: {self.batch_size} (based on {model_class.__name__})"
                )
            except StopIteration:
        ...
        for i, model in enumerate(models):
            ...
            if load_rate_logger.log_count < 5:
                if (
                    current_count % 100 == 0
                    and current_count - last_logged_count >= 100
                ):
                    should_log = True
            else:
            ...
            if should_log and load_rate_logger.should_log():
                load_rate_logger.log(
                    f"[Run {run.id}] Processed {current_count:,} models from iterator..."
                )
            ...
            if len(batch) >= self.batch_size:
                ...
                if len(recent_batch_times) > 10:
                    ...
                    if avg_time > 1.0 and self.adaptive_batch:  # Slower than 1 second
                        ...
                        if self.batch_size != old_batch_size:
                            logger.info(
                                f"[Run {run.id}] Reduced batch size to {self.batch_size} (avg: {avg_time:.2f}s)"
                            )
                ...
                run.checkpoint = {
                    ...
                    "total_expected_records": run.checkpoint.get(
                ...
                if (i + 1) <= (self.batch_size * 5) or (i + 1) % (
                    self.batch_size * 10
                ) == 0:
                    ...
                    if self.update_mode:
                        logger.info(
                            f"[Run {run.id}] Processed {i + 1:,} records, updated {run.records_updated:,} ({rate:,.0f} rec/sec{eta_msg})"
                        )
                    else:
                        logger.info(
                            f"[Run {run.id}] Loaded {i + 1:,} records ({rate:,.0f} rec/sec{eta_msg})"
                        )
        ...
        if self.update_mode:
            logger.info(
                f"[Run {run.id}] Database update completed: {run.records_updated:,} records updated, {run.records_skipped:,} skipped in {stage_duration:.2f}s"
            )
        else:
            logger.info(
                f"[Run {run.id}] Database load completed: {run.records_created:,} records in {stage_duration:.2f}s"
            )
    ...
    def _log_performance_summary(
        self, run: IngestRun, total_duration: float, total_expected: int | None
    ):
        ...
        for stage_name, duration in self.stage_timings.items():
            ...
            logger.info(
                f"[Run {run.id}]   {stage_name.capitalize()}: {duration:.2f}s ({percentage:.1f}%)"
            )
        ...
        if self.stage_timings:
            ...
            bottleneck_pct = (
                (bottleneck_time / total_duration * 100) if total_duration > 0 else 0
            )
            logger.info(
                f"[Run {run.id}]   Bottleneck: {bottleneck_stage} stage ({bottleneck_pct:.1f}% of total)"
            )
        ...
        if self.update_mode:
            ...
            logger.info(
                f"[Run {run.id}]   Records: {run.records_updated:,} updated, {run.records_skipped:,} skipped, {run.records_failed:,} failed"
            )
        else:
            ...
            if total_expected and run.records_created > 0:
                accuracy = (
                    (run.records_created / total_expected * 100)
                    if total_expected > 0
                    else 0
                )
                logger.info(
                    f"[Run {run.id}]   Records: {run.records_created:,} created (estimated: {total_expected:,}, {accuracy:.1f}%)"
                )
    ...
    def _log_pipeline_eta(self):
        ...
        if remaining_sources > 0:
            ...
            logger.info(
                f"Pipeline Progress: {self.completed_sources_count}/{self.pending_sources_count} sources completed"
            )
            logger.info(
                f"Pipeline ETA: {estimated_remaining / 60:.1f} min remaining ({estimated_total / 60:.1f} min total)"
            )
    ...
    def _insert_batch_to_db_only(self, batch: list, run: IngestRun):
        ...
        try:
            ...
            if self.prefilter_existing:
                ...
                if source_file:
                    ...
                    if not batches_by_type:
                        ...
                        if (
                            not hasattr(self, "_skip_logged")
                            or (time.time() - getattr(self, "_last_skip_log_time", 0))
                            > 30
                        ):
                            logger.debug(
                                f"[Run {run.id}] All records already exist, skipping batch"
                            )
            ...
            @handle_unrecoverable_errors(
                ...
                logger_instance=logger,
                on_unrecoverable=lambda e: setattr(
                    run, "records_failed", run.records_failed + len(batch)
                ),
            )
                    ...
                    for model_class, type_batch in batches_by_type.items():
                        if self.use_copy:
                            ...
                            model_class.objects.bulk_create(
                                type_batch, ignore_conflicts=True
                            )
        ...
        try:
            ...
            if source_file_date_str:
                try:
                    ...
                    logger.warning(
                        f"[Run {run.id}] Could not parse source_file_date from checkpoint: {source_file_date_str}"
                    )
            ...
            for model_class, type_batch in batches_by_type.items():
                ...
                new_records, existing_records_dict = _identify_new_and_existing(
                    type_batch
                )
                ...
                for incoming_record in type_batch:
                    ...
                    if case_number in existing_records_dict:
                        ...
                        incoming_date = getattr(
                            incoming_record, "source_file_date", None
                        )
                        existing_date = getattr(
                            existing_record, "source_file_date", None
                        )
                        ...
                        if self._is_newer_than_existing(incoming_date, existing_date):
                            ...
                            for field in incoming_record._meta.fields:
                                ...
                                if isinstance(field, models.DateTimeField) and (
                                    ...
                                    continue
                                incoming_val = getattr(
                                    incoming_record, field.name, None
                                )
                                if (
                                    field.name in MERGE_PRESERVE_FIELDS
                                    and incoming_val is None
                                ):
                                    existing_val = getattr(
                                        existing_record, field.name, None
                                    )
    ...
    def _is_newer_than_existing(
        self, incoming_date: datetime | None, existing_date: datetime | None
    ) -> bool:
    ...
    def _update_batch_to_db(self, batch: list, run: IngestRun):
        ...
        try:
            ...
            if not hasattr(batch[0], "case_number"):
                logger.warning(
                    f"[Run {run.id}] Model {model_class.__name__} has no case_number field, cannot update"
                )
                run.records_failed += len(batch)
                return
            ...
            existing_queryset = model_class.objects.filter(
                case_number__in=unique_case_numbers
            )
            ...
            for new_record in batch:
                ...
                if (
                    case_number in existing_records
                    and case_number not in matched_case_numbers
                ):
            ...
            if unmatched_count > 0:
                ...
                logger.debug(
                    f"[Run {run.id}] {unmatched_count} records in batch not found in database (skipped)"
                )
    ...
    def _generate_version_tag(self, run: IngestRun) -> str:
        ...
        domain = (
            run.source.domain.value
            if hasattr(run.source.domain, "value")
            else run.source.domain
        )
        source_type = (
            run.source.source_type.value
            if hasattr(run.source.source_type, "value")
            else run.source.source_type
        )
    ...
    def _link_records_to_version(self, run: IngestRun, version: IngestVersion):
        ...
        if not source_file:
            logger.warning(
                f"[Run {run.id}] No filepath in checkpoint, cannot link records to version"
            )
            return
        ...
        if run.source.source_type in [SourceType.LCA, SourceType.PERM]:
            updated = SalaryRecord.objects.filter(
                source_file=source_file, ingest_version__isnull=True
            ).update(ingest_version=version)
            logger.info(
                f"[Run {run.id}] Linked {updated} salary records to version {version.version_tag}"
            )
        ...
        # Cutoff dates are linked via bulletin publication date extracted from source URL
            ...
            if bulletin_date:
                ...
                if bulletin_date:
                    ...
                    logger.info(
                        f"[Run {run.id}] Linked {updated} cutoff dates to version {version.version_tag}"
                    )
                else:
                    logger.warning(
                        f"[Run {run.id}] Could not parse publication_date from metadata"
                    )
            else:
                logger.warning(
                    f"[Run {run.id}] No publication_date in source metadata for bulletin linking"
                )

    def _validate_post_ingest(
        self, plugin: DataSourcePlugin, run: IngestRun
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            if result.passed:
                logger.info(
                    f"[Run {run.id}] Validation passed in {validation_duration:.2f}s"
                )
            else:
                logger.error(
                    f"[Run {run.id}] Validation failed in {validation_duration:.2f}s: {len(result.errors)} error(s)"
                )
            ...
            return ValidationResult(
                passed=False, errors=[f"Validation exception: {str(e)}"], warnings=[]
            )

    def _validate_copy_preflight(
        self, records: list, fields: list, model_class
    ) -> None:
        ...
        if failures:
            ...
            raise ValueError(
                "COPY preflight failed: non-numeric DecimalField values detected"
            )
    ...
    def _bulk_insert_via_copy(self, records: list, model_class):
        ...
        logger.info(
            f"🔍 COPY field order for {model_class.__name__}: {field_names_str}"
        )
        ...
        for record in records:
            ...
            if record_count <= 3:
                logger.info(
                    f"Record {record_count} ({record_id}): wage_from={getattr(record, 'wage_from', None)}, wage_to={getattr(record, 'wage_to', None)}, wage_unit={getattr(record, 'wage_unit', None)}"
                )
            ...
            for field in fields:
                ...
                if hasattr(value, "value"):
                    ...
                    if record_count <= 3 and field.attname in (
                        "wage_unit",
                        "wage_from",
                        "wage_to",
                        ...
                        logger.info(
                            f"  ⚙️  {field.attname}: {original_value!r} -> {value!r}"
                        )
            ...
            if (
                wage_fields_present
                and len(values) > schema.wage_to
                and values[schema.wage_to] == "year"
            ):
                ...
                logger.error(
                    ...
                    record_id,
                    schema.wage_from,
                    values[schema.wage_from]
                    if len(values) > schema.wage_from
                    else None,
                    schema.wage_to,
                    values[schema.wage_to],
                    schema.wage_unit,
                    values[schema.wage_unit]
                    if len(values) > schema.wage_unit
                    else None,
                    copy_line[:300],
                )
        ...
        logger.info(
            f"💾 Saved COPY buffer to {debug_path} ({len(buffer_content)} bytes, {len(records)} records)"
        )
        ...
        with connection.cursor() as cursor:
            cursor.copy_from(
                buffer, model_class._meta.db_table, columns=field_names, null="\\N"
            )
        ...
        if source.domain == DataDomain.VISA_BULLETIN:
            try:
                ...
                if not bulletin_date and run.checkpoint:
                    ...
                    if filepath_str:
                        ...
                        date_match = re.search(
                            r"visa-bulletin-for-(\w+)-(\d{4})", filename, re.IGNORECASE
                        )
                        if date_match:
                            from datetime import datetime
                ...
                if bulletin_date:
                    ...
                    logger.debug(
                        f"[Run {run.id}] Post-ingest hook: Could not determine bulletin date for evaluation"
                    )
            except Exception as e:
                logger.warning(
                    f"[Run {run.id}] Failed to trigger VQS evaluation hook: {e}"
                )
