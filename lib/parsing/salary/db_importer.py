...
def _create_salary_record(
    ...
    is_worksite = (
        source_file.startswith('LCA_Worksites') or
        '_Worksites_' in source_file or
...
class RowProcessResult:
    ...
    def __init__(self, record: SalaryRecord | None = None, skipped: bool = False, error: bool = False, rejected: bool = False, rejection_reason: str | None = None):
        self.record = record
...
def _batch_insert_records(records: list[SalaryRecord], imported: int, batch_size: int, batch_num: int = 0) -> tuple[int, float, float]:
    """
...
def import_csv_file(
    ...
    if excel_timing_info:
        ...
        logger.info(f"  Dict conversion: {excel_dict_convert_time:.2f}s ({excel_dict_convert_time/total_time*100:.1f}%)")
    ...
    logger.info(f"  Existing cases lookup: {existing_cases_time:.2f}s ({existing_cases_time/total_time*100:.1f}%)")
    if row_count > 0:
