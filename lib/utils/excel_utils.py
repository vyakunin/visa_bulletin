...
def read_excel_streaming(
    filepath: Path, start_row: int = 2, read_only: bool = True, data_only: bool = True
) -> Iterator[dict]:
    """
...
def read_excel_row(
    filepath: Path, row_number: int, read_only: bool = True, data_only: bool = True
) -> dict | None:
    """
    ...
    rows = read_excel_rows(
        filepath, [row_number], read_only=read_only, data_only=data_only
    )
