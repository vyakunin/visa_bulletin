...
def _get_file_stats_impl(
    filepath: Path, logger_instance: logging.Logger | None = None
) -> dict:
    """
    ...
    if cache_key in cache:
        ...
        if cached_value is not None:
            ...
            if isinstance(cached_value, int):
                log.debug(
                    f"Using cached row count for {filepath.name}: {cached_value:,}"
                )
                return cached_value
                ...
                if row_count is not None:
                    log.debug(
                        f"Using cached row count for {filepath.name}: {row_count:,}"
                    )
                    return row_count
...
def get_fiscal_year_from_filename(
    filename: str, fallback_url: str | None = None
) -> int | None:
    """
...
def get_fiscal_year_from_datasource(
    ...
    fiscal_year = get_fiscal_year_from_filename(
        source_file, fallback_url=data_source.url
    )

    # Strategy 2: If URL is file:// or reimport://, try advanced strategies
    if fiscal_year is None and (
        data_source.url.startswith("file://")
        or data_source.url.startswith("reimport://")
    ):
        ...
        if data_source.local_file_path:
            ...
            for alt_source in other_sources:
                alt_fiscal_year = get_fiscal_year_from_filename(
                    source_file, fallback_url=alt_source.url
                )
                if alt_fiscal_year:
                    logger_instance.debug(
                        f"Found fiscal year from alternative DataSource {alt_source.id}: {alt_source.url}"
                    )
        ...
        for run in runs:
            ...
            if (
                run.source.url
                and not run.source.url.startswith("file://")
                and not run.source.url.startswith("reimport://")
            ):
                fiscal_year = get_fiscal_year_from_filename(
                    source_file, fallback_url=run.source.url
                )
                if fiscal_year:
                    return fiscal_year
            ...
            if run.checkpoint and isinstance(run.checkpoint, dict):
                ...
                if checkpoint_filepath:
                    ...
                    if checkpoint_filename != source_file:
                        logger_instance.debug(
                            f"Trying checkpoint filename: {checkpoint_filename}"
                        )
        ...
        if data_source.metadata:
            original_filename = data_source.metadata.get(
                "original_filename"
            ) or data_source.metadata.get("filename")
...
def get_source_file_date(filepath: Path, data_source: "DataSource") -> datetime | None:
    ...
    fiscal_year = get_fiscal_year_from_filename(
        filepath.name, fallback_url=data_source.url
    )
