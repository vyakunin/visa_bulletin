...
class FileComparisonResult:
    ...
    reason: str | None = (
        None  # Reason for missing records (e.g., "all_duplicates", "worksite_file", "not_imported")
    )
...
def get_file_record_counts() -> dict[str, dict]:
    ...
    salary_counts = SalaryRecord.objects.values("source_file", "visa_program").annotate(
        count=Count("id")
    )
    ...
    for item in salary_counts:
        ...
        if source_file not in result:
            result[source_file] = {"total": 0, "by_program": {}}
        result[source_file]["total"] += item["count"]
        result[source_file]["by_program"][item["visa_program"]] = (
            result[source_file]["by_program"].get(item["visa_program"], 0)
            + item["count"]
        )

    # Get WorksiteRecord counts (worksite files create WorksiteRecord entries)
    worksite_counts = WorksiteRecord.objects.values(
        "source_file", "visa_program"
...
def get_db_counts_by_year() -> dict[int, int]:
    ...
    worksite_by_year = (
        WorksiteRecord.objects.values("fiscal_year")
        .annotate(count=Count("id"))
        .order_by("fiscal_year")
    )
...
def _analyze_missing_records_reason(
    filepath: Path, filename: str, file_rows: int, db_count: int
) -> str | None:
    """
    ...
    try:
        ...
        if case_numbers_sample:
            ...
            if existing_count == len(case_numbers_sample):
                ...
                return (
                    f"all_duplicates (sampled {len(case_numbers_sample)}/{file_rows:,})"
                )
...
def compare_file_counts(
    ...
    for filepath in sorted(data_files):
        ...
        if not return_all:
            include_file = (
                abs(discrepancy) >= min_discrepancy_threshold
                or abs(discrepancy_pct) >= min_discrepancy_pct
            )
        ...
        if include_file:
            ...
            if db_count == 0 and file_rows > 0:
                reason = _analyze_missing_records_reason(
                    filepath, filename, file_rows, db_count
                )
