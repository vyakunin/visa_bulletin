...
# The qualifying-slug set is a per-occupation COUNT over the corpus; cache it so
# per-Googlebot sitemap fetches (which bypass the rendered-page cache) don't rerun
# it. The refresh pipeline's cache.clear() on each ingest refreshes it.
...
def occupation_filing_count(occ: Occupation) -> int:
    """Total qualifying filings for the occupation — the cheap 404-gate input."""
    return occupation_base_qs(occ).count()
