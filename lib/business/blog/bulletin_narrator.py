...
class BulletinNarrator:
    ...
    def _get_previous_bulletin(self, target_date: date) -> Optional[Bulletin]:
        return Bulletin.objects.filter(publication_date__lt=target_date).order_by("-publication_date").first()

    def _get_prediction(self, target_date: date) -> Optional[PredictedBulletin]:
        return PredictedBulletin.objects.filter(target_bulletin_month=target_date).first()

    def _analyze_movements(self, current: Bulletin, previous: Optional[Bulletin]) -> Dict:
        """
        ...
        priority_checks = [
            ...
            ("F1", Country.Philippines, "F1 Philippines") if hasattr(Country, 'Philippines') else ("F1", "PH", "F1 Philippines"),
        ]
    ...
    def _analyze_surprises(self, current: Bulletin, prediction: Optional[PredictedBulletin]) -> List[Dict]:
        """
        ...
        for actual in current.cutoff_dates.filter(action_type="final_action"):
            ...
            if pred_date:
                ...
                if abs(delta) > 14:
                    ...
                    if delta > 0:
                        item["narrative"] = f"Better than expected. We predicted {pred_date} but it moved to {actual.cutoff_date}."
                    else:
                        item["narrative"] = f"Worse than expected. We predicted {pred_date} but it only reached {actual.cutoff_date}."
