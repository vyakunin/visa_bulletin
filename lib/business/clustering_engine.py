...
class EntityClusteringConfig(Protocol[EntityType, ClusterType]):
    ...
    def should_apply_additional_filter(
        self, entity1: EntityType, entity2: EntityType, norm1: str, norm2: str
    ) -> bool:
        """
...
def match_entities(
    ...
    if not config.should_apply_additional_filter(entity1, entity2, norm1, norm2):
        return MatchResult(
            is_match=False, confidence=0.0, reason="Failed entity-specific filter"
        )
    ...
    if norm1 == norm2:
        ...
        return MatchResult(
            is_match=True, confidence=1.0, reason="Exact normalized match"
        )
...
def should_auto_cluster(
    ...
    return MatchResult(
        is_match=False, confidence=result.confidence, reason=result.reason
    )
...
def assign_to_cluster(
    ...
    if bucket_index is not None:
        ...
        all_entities = EntityModel.objects.exclude(id=entity.id).select_related(
            "canonical_cluster"
        )
        similar_entities = []
            ...
            if result.is_match:
                ...
                if match_result.confidence >= 0.6:
                    config.create_review_entry(
                        ...
                        match_result.reason
                        or f"Fuzzy match: {match_result.confidence:.3f}",
                    )

    if hasattr(config, "create_cluster"):
        cluster = config.create_cluster(entity)
    else:
        cluster = ClusterModel.objects.create(canonical_name=entity.name)
