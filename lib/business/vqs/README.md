...
- **direction_metrics.py** – `direction_metrics(pairs)` → `DirectionMetrics`: direction scoring with its null baseline. ~90% of EB moves are advances, so plain conditional direction accuracy is largely a restatement of that class mix — `cond_up_rate` reports the score an "always advance" constant classifier gets, and `bal_direction_acc` (mean of advance and retrogression recall) is the base-rate-proof skill metric (constant classifier = 50%). See §26.
