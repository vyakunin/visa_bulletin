...
def expert_fy_reset(
    ...
    if target_month == 10:  # Predict October retrogression
        retro_days = get_median_october_retrogression(visa_class, country, action_type, knowledge_date)
        if retro_days > 0: # Correction: get_median_october_retrogression returns positive days of retrogression
...
def expert_vol_gated(
    ...
    seasonal_pred = expert_seasonal_median(visa_class, country, action_type, knowledge_date)
...
def expert_supply_aware(
    ...
    from datetime import timedelta, date # date is already imported at module level, but good for clarity
    ...
    return current_cutoff # If pace is not available or not positive, return persistence.
...
def expert_dos_historical(
    ...
    hist_move = get_seasonal_prediction(visa_class, country, action_type, knowledge_date, target_month)
    if hist_move is None:
        return current_cutoff # Fallback to persistence
        
    # 3. Get historical median issuance (visas)
    hist_issuance = get_historical_issuance_median(visa_class, country, target_month, knowledge_date, facts)
    ...
    allocator = SupplyAllocator()
    current_supply = allocator.get_supply(visa_class, country, bulletin_date, knowledge_date).total
