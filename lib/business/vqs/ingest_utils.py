...
def normalize_month(m_str: str) -> int:
    try:
        ...
        if m_str in m_map: return m_map[m_str]
        m_map_abbr = {name: i for i, name in enumerate(calendar.month_abbr) if name}
        if m_str in m_map_abbr: return m_map_abbr[m_str]
    except:
        pass
    return 0
...
def normalize_visa_class_inventory(v_str: str) -> str:
    ...
    v = v_str.upper().strip()
    if "1ST" in v or "E1" in v: return "1st"
    if "2ND" in v or "E2" in v: return "2nd"
    if "3RD" in v or "E3" in v: return "3rd"
    if "4TH" in v: return "4th"
    if "5TH" in v: return "5th"
    return "Unknown"
