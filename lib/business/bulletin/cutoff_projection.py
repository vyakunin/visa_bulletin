...
def _calculate_linear_regression_coefficients(
    valid_points: list[tuple[date, date]], epoch: date
) -> tuple[float, float] | None:
    """
    Calculate linear regression slope and intercept for historical data

    Returns:
        Tuple of (slope, intercept) or None if regression cannot be calculated
    """
...
def _project_from_regression(
    ...
    """
    Project when submission_date will be reached using regression coefficients

    Returns:
        dict with projection info or None if already reached
    """
...
def calculate_historical_linear_regression(
    ...
    return _project_from_regression(
        slope, intercept, submission_date, last_pub_date, epoch
    )
