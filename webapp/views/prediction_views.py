...
def prediction_detail(request: HttpRequest, year: int, month: int, category: str = "employment_based") -> HttpResponse:
    """Detailed view of a specific month's data (Prediction + Bulletin)."""
    target_date = date(year, month, 1)
    
    # Try to find prediction
    bulletin = PredictedBulletin.objects.filter(target_bulletin_month=target_date).first()
    ...
    if not bulletin and not actual_bulletin:
        get_object_or_404(PredictedBulletin, target_bulletin_month=target_date) # Trigger 404
    ...
    prev_bulletin = PredictedBulletin.objects.filter(target_bulletin_month__lt=target_date).order_by("-target_bulletin_month").first()
    prev_actual = Bulletin.objects.filter(publication_date__lt=target_date).order_by("-publication_date").first()
    
    next_bulletin_obj = PredictedBulletin.objects.filter(target_bulletin_month__gt=target_date).order_by("target_bulletin_month").first()
    next_actual = Bulletin.objects.filter(publication_date__gt=target_date).order_by("publication_date").first()

    # Pick the closest prev/next
    nav_prev = None
    if prev_bulletin and prev_actual:
        nav_prev = max(prev_bulletin.target_bulletin_month, prev_actual.publication_date)
    elif prev_bulletin:
    ...
    if next_bulletin_obj and next_actual:
        nav_next = min(next_bulletin_obj.target_bulletin_month, next_actual.publication_date)
    ...
    last_actual_bulletin = Bulletin.objects.filter(publication_date=last_actual_month).first()
    last_actual_cutoffs = {}
    ...
    countries = [
        Country.ALL, Country.CHINA, Country.INDIA, Country.MEXICO, Country.PHILIPPINES
    ]
    ...
    for vc in classes:
        matrix[vc] = {}
        for c in countries:
            matrix[vc][c.value] = {"final_action": {"predicted": None, "actual_date": None}, "filing": {"predicted": None, "actual_date": None}}
        ...
        for cutoff in bulletin.cutoffs.all():
            if cutoff.visa_class in matrix:
                if cutoff.country in matrix[cutoff.visa_class]:
                    atype = "filing" if "filing" in cutoff.action_type else "final_action"
                    ...
                    if cutoff.predicted_date:
                        cutoff.formatted_date = cutoff.predicted_date.strftime("%d %b %Y")
                    
                    matrix[cutoff.visa_class][cutoff.country][atype]["predicted"] = cutoff
        ...
        for c in countries:
            for atype in ["final_action", "filing"]:
                ...
                if key in current_actual_cutoffs:
                    ...
                    matrix[vc][c.value][atype]["formatted_actual"] = d.strftime("%d %b %Y") if d else None
    ...
    formatted_title = f'{category_label} Predictions for {target_date.strftime("%B %Y")}' if bulletin else f'{category_label} Bulletin: {target_date.strftime("%B %Y")}'
