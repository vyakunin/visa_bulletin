...
def main():
    ...
    pip_check = subprocess.run(
        [system_python, "-m", "pip", "--version"], capture_output=True, text=True
    )
    ...
    if not pip_compile_script:
        check_result = subprocess.run(
            [system_python, "-c", "import pip_tools"], capture_output=True, text=True
        )
    else:
        check_result = subprocess.CompletedProcess(
            [], 0, "", ""
        )  # Script exists, skip check
    ...
    if check_result.returncode != 0:
        ...
        if install_result.returncode != 0:
            print(
                "ERROR: Failed to install pip-tools. Install manually with:",
                file=sys.stderr,
            )
    ...
    user_site_result = subprocess.run(
        [system_python, "-m", "site", "--user-site"], capture_output=True, text=True
    )
    user_site = (
        user_site_result.stdout.strip() if user_site_result.returncode == 0 else None
    )
    ...
    if result.returncode == 0:
        ...
        print(
            f"ERROR: pip-compile failed with exit code {result.returncode}",
            file=sys.stderr,
        )
        return 1
