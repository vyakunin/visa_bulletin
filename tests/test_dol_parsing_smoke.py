...
class TestDolParsingSmokeTest(unittest.TestCase):
    ...
    def _validate_file(
        self, filename: str, domain: str, source_type: str, sample_rows: int = 5
    ) -> dict:
        """
                ...
                try:
                    ...
                    if model_instance:
                        ...
                        if not getattr(model_instance, "case_number", None):
                            errors.append(
                                f"{filename} row {idx + 1}: Missing case_number"
                            )
                        if hasattr(model_instance, "employer_name") and not getattr(
                            model_instance, "employer_name", None
                        ):
                            errors.append(
                                f"{filename} row {idx + 1}: Missing employer_name"
                            )
                        if not getattr(model_instance, "job_title", None):
                            errors.append(
                                f"{filename} row {idx + 1}: Missing job_title"
                            )
                except Exception as e:
                    errors.append(
                        f"{filename} row {idx + 1}: Transform failed - {str(e)}"
                    )
    ...
    def test_perm_fy2020(self):
        ...
        file_path = (
            self.workspace_dir / "data" / "salary" / "dol_data" / "PERM_FY2020.xlsx"
        )
        if not file_path.exists():
        ...
        self.assertTrue(
            result["success"], f"PERM FY2020 parsing failed: {result['errors']}"
        )
        self.assertGreater(
            result["transformed_count"], 0, "No records transformed from PERM FY2020"
        )
    ...
    def test_perm_fy2024_q4(self):
        ...
        result = self._validate_file(
            "PERM_Disclosure_Data_FY2024_Q4.xlsx", "dol", "perm"
        )
        self.assertTrue(
            result["success"], f"PERM FY2024 Q4 parsing failed: {result['errors']}"
        )
        self.assertGreater(
            result["transformed_count"], 0, "No records transformed from PERM FY2024 Q4"
        )
        self.assertEqual(result["parsed_count"], 5, "Expected 5 parsed records")
    ...
    def test_lca_fy2020(self):
        ...
        self.assertTrue(
            result["success"], f"LCA FY2020 parsing failed: {result['errors']}"
        )
        self.assertGreater(
            result["transformed_count"], 0, "No records transformed from LCA FY2020"
        )
        self.assertEqual(result["parsed_count"], 5, "Expected 5 parsed records")
    ...
    def test_lca_fy2024_q4(self):
        ...
        self.assertTrue(
            result["success"], f"LCA FY2024 Q4 parsing failed: {result['errors']}"
        )
        self.assertGreater(
            result["transformed_count"], 0, "No records transformed from LCA FY2024 Q4"
        )
        self.assertEqual(result["parsed_count"], 5, "Expected 5 parsed records")
