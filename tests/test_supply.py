...
class TestSupplyModule(unittest.TestCase):
    ...
    def test_cascade_model(self, mock_supply, mock_cutoff):
        ...
        bonus = model.estimate_cascade_bonus("2nd", Country.INDIA.value, sim_month, knowledge_date)
        # Expect 35% of 1000 = 350
        self.assertEqual(bonus, 350)
        
        # Test EB1 (receives nothing)
        bonus = model.estimate_cascade_bonus("1st", Country.INDIA.value, sim_month, knowledge_date)
        ...
        bonus = model.estimate_cascade_bonus("3rd", Country.INDIA.value, sim_month, knowledge_date)
