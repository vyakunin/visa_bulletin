...
class TestClusteringRegression(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        possible_paths = [
            ...
            / "data"
            / "clustering_examples.jsonl",  # Local execution
        ...
        cls.true_positives = [
            ex for ex in cls.examples if ex.get("ground_truth") == "same"
        ]
        cls.true_negatives = [
            ex for ex in cls.examples if ex.get("ground_truth") == "different"
        ]
        ...
        non_generic_cases = [
            {
                ...
                "emp2": Employer(
                    name="EMC CORPORATION", city="ROUND ROCK", state="TEXAS"
                ),
                "should_match": True,
        ...
        for case in non_generic_cases:
            with self.subTest(emp1=case["emp1"].name, emp2=case["emp2"].name):
                ...
                should_cluster, cluster_confidence, cluster_reason = (
                    should_auto_cluster(emp1, emp2, threshold=0.95)
                )
                self.assertTrue(
                    should_cluster,
    ...
    def test_case_variations_should_match(self):
        ...
        case_variation_cases = [
            ...
            {
                "emp1": Employer(
                    name="JP MORGAN CHASE & CO", city="New York", state="NY"
                ),
                "emp2": Employer(
                    name="JP Morgan Chase & Co", city="New York", state="NY"
                ),
