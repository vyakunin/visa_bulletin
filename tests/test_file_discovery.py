...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["CASE_NUMBER", "WAGE_RATE_OF_PAY_FROM", "WORKSITE_CITY", "WORKSITE_STATE"],
            "H1B",
        ),
...
def test_all_files_discovered():
    ...
    # Verify no errors
    assert len(error_files) == 0, f"Files with errors: {error_files}"
    ...
    assert coverage >= 90, f"Coverage too low: {coverage:.1f}% (expected >= 90%)"
...
def test_appendix_files_detected_as_unknown():
    ...
    assert detected is None, f"Appendix A files should be unknown, got {detected}"
