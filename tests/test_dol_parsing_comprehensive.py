...
class TestDolParsingComprehensive(TestCase):
    ...
    def _test_transform(
        ...
        filename: str,
        sample_row: dict,
        expect_type: ExpectedOutputType,
    ):
        """
        ...
        if expect_type == ExpectedOutputType.SALARY_RECORD:
            self.assertIsInstance(
                result, SalaryRecord, f"{filename} should return SalaryRecord"
            )
            self.assertIsNotNone(result.case_number, f"{filename}: Missing case_number")
            self.assertIsNotNone(result.job_title, f"{filename}: Missing job_title")
        elif expect_type == ExpectedOutputType.WORKSITE_RECORD:
            self.assertIsInstance(
                result, WorksiteRecord, f"{filename} should return WorksiteRecord"
            )
            self.assertIsNotNone(result.case_number, f"{filename}: Missing case_number")
        elif expect_type == ExpectedOutputType.NONE:
            self.assertIsNone(
                result, f"{filename} should return None (supplemental data)"
            )
        else:
    ...
    def test_lca_fy2009_format(self):
        ...
        self._test_transform(
            ...
            "H-1B_FY2009.xlsx",
            sample_row,
            ExpectedOutputType.WORKSITE_RECORD,
        )  # I-200 = worksite
    ...
    def test_lca_fy2020_format(self):
        ...
        self._test_transform(
            ...
            sample_row,
            ExpectedOutputType.SALARY_RECORD,
        )  # I-203 = regular LCA
