    ...
    def test_golden_set_normalizations(self):
        ...
        golden_set = [
            ...
            (
                ...
                "software engineer",
                "ii",
            ),  # Level marker stripped; level in experience_level
            (
            ...
            (
                "Manager 2, Supply Chain",
                "manager supply chain",
                "ii",
            ),  # Level marker detected before digit removal
            (
                "Director 3, Engineering",
            ...
            (
                ...
                "programmer analyst",
                "",
            ),  # programmer NOT converted (only as complete title)
            ("Registered Nurse", "nurse", ""),  # RN → nurse
            (
                "Physician Assistant",
                "physician assistant",
                "",
            ),  # physician NOT converted (only as complete title)
        ]
        ...
        for original, expected_norm, expected_level in golden_set:
            with self.subTest(title=original):
                ...
                self.assertEqual(
                    normalized, expected_norm, f"Normalization failed for '{original}'"
                )
                self.assertEqual(
                    level, expected_level, f"Level extraction failed for '{original}'"
                )
    ...
    def test_special_characters_removed(self):
        ...
        test_cases = [
            (
                ...
                "software engineer full stack",
            ),  # Meaningful parentheticals kept
            (
                "Data Scientist - ML",
                "data scientist ml",
            ),  # ml NOT expanded (only as complete title)
